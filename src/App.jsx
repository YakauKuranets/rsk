...
const handleRunNexusProtocol = async (terminal) => {
    ...
    try {
      // Отправляем приказ в асинхронное Ядро
      const result = await invoke('run_nexus_protocol', { ip: terminal.host });

      // Обрабатываем новые фоновые статусы
      if (result.status === 'COMMAND_ISSUED') {
        alert(`📡 ПРИКАЗ ОТПРАВЛЕН 📡\n\nЯдро: ${result.message}\n\nСпецназ начал штурм в фоновом режиме. Смотрите логи в терминале!`);
      } else if (result.status === 'PWNED') {
        alert(`🔥 ДОСТУП ПОЛУЧЕН 🔥\n\nIP: ${result.ip}\nПорт: ${result.port}`);
      } else {
        alert(`✖ ЦЕЛЬ ОТБРОШЕНА\n\nПричина: ${result.reason || 'Неизвестно'}`);
      }
    } catch (err) {
            ...
            <div style={{ backgroundColor: '#0a0a0c', border: '1px solid #00f0ff', padding: '10px', width: '220px', borderRadius: '4px' }}>
              ...
              {t.type !== 'hub' && (
                <button
                  ...
                  style={{ width: '100%', backgroundColor: '#ff003c', color: '#000', border: 'none', padding: '8px', cursor: 'pointer', fontSize: '11px', fontWeight: 'bold', letterSpacing: '1px' }}
                >
                  ☢ ПРОТОКОЛ NEMESIS
                </button>
              )}
