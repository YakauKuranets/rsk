...
// =============================================================================
// 🛠 БРОНЕБОЙНЫЙ ФИКС ИКОНОК LEAFLET ДЛЯ VITE
// =============================================================================
...
export default function App() {
  ...
  const [nexusTargetIp, setNexusTargetIp] = useState('');
  ...
  useEffect(() => {
    ...
    const fetchLogs = async () => {
      try {
        ...
        if (!disposed) {
          ...
          // Detect nexus completion
          if (nexusRunning) {
            const hasComplete = logs.some(l => l.includes('[COMPLETE]') && l.includes(nexusTargetIp));
            const hasFailed = logs.some(l => l.includes('[FAILED]') && l.includes(nexusTargetIp));
            if (hasComplete || hasFailed) {
              setNexusRunning(false);
              setRadarStatus(hasComplete
                ? `✅ NEMESIS ЗАВЕРШЕН: ${nexusTargetIp}`
                : `❌ NEMESIS ПРОВАЛ: ${nexusTargetIp}`
              );
            }
          }
    ...
    fetchLogs();
    const timer = setInterval(fetchLogs, nexusRunning ? 800 : 2000);
    return () => {
      disposed = true;
      clearInterval(timer);
    };
  }, [nexusRunning, nexusTargetIp]);
  ...
  const handleRunNexusProtocol = async (terminal) => {
    setLoading(true);
    setNexusRunning(true);
    setNexusTargetIp(terminal.host);
    setRadarStatus(`☢️ ИНИЦИАЛИЗАЦИЯ ПРОТОКОЛА NEMESIS: ${terminal.host}...`);
    try {
      const result = await invoke('run_nexus_protocol', {
        ip: terminal.host,
        login: terminal.login || 'admin',
        pass: terminal.password || '',
      });

      if (result.status === 'COMMAND_ISSUED') {
        setRadarStatus(`☢️ NEMESIS ACTIVE: ${terminal.host} — ${result.message}`);
        // НЕ показываем alert — прогресс отображается в логах в реальном времени
      } else {
        setRadarStatus(`✖ NEMESIS: ${result.reason || 'Неизвестная ошибка'}`);
      }
    } catch (err) {
      alert(`Ошибка протокола Nemesis: ${err}`);
      setNexusRunning(false);
    } finally {
      setLoading(false);
      // nexusRunning остается true — сбросится когда в логах появится COMPLETE/FAILED
    }
  };
...
const handleSecurityAudit = async () => {
  ...
  <MapContainer center={mapCenter} zoom={13} style={{ height: '100%', width: '100%', minHeight: '500px' }} zoomControl={false}>
    <MapController center={mapCenter} />
    <TileLayer url="https://{s}.basemaps.cartocdn.com/dark_all/{z}/{x}/{y}{r}.png" />

    {/* ПРАВИЛЬНЫЙ РЕНДЕР МЕТОК (MARKER) С КАМЕРАМИ И СТРИМАМИ */}
    {filteredTargets.map(t => {
      // Защита: если координат нет, не пытаемся рисовать
      if (!t.lat || !t.lng) return null;

      return (
        <Marker key={`map_${t.id}`} position={[t.lat, t.lng]}>
          <Popup>
            <div style={{ backgroundColor: '#0a0a0c', border: '1px solid #00f0ff', padding: '10px', width: '220px', borderRadius: '4px' }}>
              <div style={{ color: t.type === 'hub' ? '#ff00ff' : '#00f0ff', fontSize: '14px', fontWeight: 'bold', marginBottom: '4px' }}>
                {t.name}
              </div>
              ...
              {/* 👇 ВОТ ЭТОТ БЛОК ГЕНЕРИРУЕТ КНОПКИ КАМЕР И ЗАПУСКАЕТ СТРИМ 👇 */}
              {t.channels && t.channels.length > 0 ? (
                <div style={{ marginBottom: '12px' }}>
                  <div style={{ color: '#888', fontSize: '9px', marginBottom: '6px', fontWeight: 'bold', letterSpacing: '1px' }}>
                    ДОСТУПНО КАМЕР: {t.channels.length}
                  </div>
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: '4px' }}>
                    {t.channels.map(ch => (
                      <button
                        key={ch.id}
                        onClick={() => handleStartStream(t, ch)}
                        style={{ backgroundColor: '#1a4a4a', color: '#00f0ff', border: '1px solid #00f0ff', padding: '4px 6px', cursor: 'pointer', fontSize: '9px', fontWeight: 'bold' }}
                      >
                        ▶ {ch.name}
                      </button>
                    ))}
                ...
                <div style={{ color: '#ff5555', fontSize: '9px', marginBottom: '10px' }}>Каналы не найдены</div>
              )}
              {/* 👆 КОНЕЦ БЛОКА КАМЕР 👆 */}
              ...
              {t.type !== 'hub' && (
                <button
                  onClick={() => handleRunNexusProtocol(t)}
                  disabled={nexusRunning}
                  style={{
                    width: '100%',
                    backgroundColor: nexusRunning && nexusTargetIp === t.host ? '#ff6600' : '#ff003c',
                    color: '#000',
                    border: 'none',
                    padding: '8px',
                    cursor: nexusRunning ? 'not-allowed' : 'pointer',
                    fontSize: '11px',
                    fontWeight: 'bold',
                    letterSpacing: '1px',
                    animation: nexusRunning && nexusTargetIp === t.host ? 'pulse 1s infinite' : 'none',
                  }}
                >
                  {nexusRunning && nexusTargetIp === t.host ? '⏳ SAND PROTOCOL...' : '☢ ПРОТОКОЛ NEMESIS'}
                </button>
              )}
            </div>
          </Popup>
  ...
  {/* ... дальше идет твой код {activeStream && (...)} */}
      ...
      <div style={{ width: '400px', backgroundColor: '#111115', borderLeft: '2px solid #ff003c', padding: '20px', overflowY: 'auto' }}>
        ...
        <h3 style={{ color: '#00f0ff', marginTop: '30px', fontSize: '0.9rem' }}>
          LIVE-ЛОГИ ЯДРА
          {nexusRunning && <span style={{ color: '#ff6600', marginLeft: '10px', fontSize: '0.75rem', animation: 'pulse 1s infinite' }}>● NEMESIS ACTIVE</span>}
        </h3>
        <div style={{ border: '1px solid #222', background: '#050505', maxHeight: '250px', overflowY: 'auto', padding: '8px' }}>
          {runtimeLogs.length === 0 && (
            <div style={{ color: '#666', fontSize: '11px' }}>Логи пока пусты</div>
          )}
          {runtimeLogs.map((line, idx) => {
            const isNexus = line.includes('[NEXUS]') || line.includes('[BRAIN]') || line.includes('[SPETSNAZ]')
              || line.includes('[CIPHER]') || line.includes('[TRANSPORT]') || line.includes('[VERDICT]')
              || line.includes('[COMPLETE]') || line.includes('[FAILED]');
            const isSand = line.includes('[SAND]') || line.includes('[SAND-STREAM]');
            const isDigest = line.includes('[STRIKE]') || line.includes('[TID:') || line.includes('[PROBE]') || line.includes('[CLOCK]');
            const isJackpot = line.includes('[JACKPOT]');
            const isComplete = line.includes('[COMPLETE]');
            const isFailed = line.includes('[FAILED]');
            const isProgress = line.includes('DOWNLOAD_PROGRESS');
            let color = '#9fefff';
            if (isJackpot) color = '#00ff44';
            else if (isComplete) color = '#00ff88';
            else if (isFailed) color = '#ff4444';
            else if (isDigest) color = '#ff66ff';
            else if (isSand) color = '#d4a843';
            else if (isNexus) color = '#ffaa00';
            else if (isProgress) color = '#88aaff';

            const isHighlight = isNexus || isSand || isDigest || isJackpot;

            return (
              <div key={`${idx}_${line}`} style={{
                color,
                fontSize: isJackpot ? '11px' : '10px',
                lineHeight: '1.4',
                marginBottom: '2px',
                fontWeight: isHighlight ? 'bold' : 'normal',
                borderLeft: isJackpot ? '3px solid #00ff44' : isDigest ? '2px solid #ff66ff' : isSand ? '2px solid #d4a843' : isNexus ? '2px solid #ffaa00' : 'none',
                paddingLeft: isHighlight ? '6px' : '0',
                fontStyle: isSand ? 'italic' : 'normal',
                opacity: isProgress ? 0.6 : 1,
                background: isJackpot ? 'rgba(0,255,68,0.08)' : 'transparent',
              }}>
                {line}
              </div>
            );
