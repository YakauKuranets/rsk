...
if missing or errors or missing_markers: status,reason='signoff_packet_blocked','safe_signoff_reference_missing'
elif blocked or notes: status,reason='signoff_packet_ready_with_notes','safe_signoff_reference_ready_with_notes'
else: status,reason='signoff_packet_ready','safe_signoff_reference_ready'

signoff_packet_status={'status':status,'reason':reason,'missing_required_inputs':sorted(set(missing)),'missing_required_markers':missing_markers,'parse_errors':errors,'triage_artifact_present':triage_present,'operator_message_ru':'Сформирован final operator signoff packet без runtime activation.'}
signoff_scope={'scope_target':'final_operator_signoff_after_briefing_signoff_prep','governance_artifact_type':'final_signoff_reference_packet','is_runtime_authorization':False,'is_execution_permit':False,'opens_implicit_runtime_transition':False,'replaces_future_runtime_phase':False,'scope_ru':'Финальная фиксация signoff readiness в design/control режиме.'}
signoff_readiness_state={'chain_completeness_state':'complete_with_notes' if notes else 'complete','marker_completeness_state':'complete' if not missing_markers else 'incomplete','governance_continuity_state':'validated','boundary_continuity_state':'validated','operator_review_completeness_state':'validated','non_execution_confirmation_state':'confirmed','runtime_closed_state':'closed','unresolved_notes_handling_ru':'Ноты допустимы при сохранении non-execution guardrails и явной документации.','not_runtime_permission_ru':'signoff readiness не означает runtime permission.'}
signoff_review_checklist=['policy/baseline reviewed','approval chain reviewed','dry-run to approval reviewed','approval to audit reviewed','audit to runtime-boundary reviewed','governance bundle reviewed','handoff governance pack reviewed','briefing/signoff-prep pack reviewed','required markers reviewed','non-execution flags reviewed','final operator signoff summary prepared']
operator_acknowledgement_rules=['acknowledge governance-only state','acknowledge runtime remains closed','acknowledge approval does not imply execution','acknowledge readiness does not imply activation','acknowledge boundary completeness does not imply runtime permission','acknowledge no hidden fallback path','acknowledge separate future runtime phase would require separate authorization']
signoff_invariants=['signoff-only governance flow','no runtime activation','no runtime execution','no graph mutation','no remediation','no hidden side effects','no policy bypass','no baseline bypass','no approval bypass','no audit bypass','no governance bypass','no briefing bypass','no silent execution fallback']
validation_rules=['signoff_packet_has_required_sections','all_required_markers_present','signoff_readiness_state_is_complete_and_consistent','signoff_review_checklist_is_complete_and_consistent','operator_acknowledgement_rules_are_complete','execution_related_flags_absent','runtime_open_flags_absent','signoff_packet_is_compatible_with_design_control_only_state']
rejection_rules=['missing_required_sections','missing_required_markers','malformed_signoff_readiness_state','malformed_signoff_review_checklist','malformed_operator_acknowledgement_rules','stale_governance_briefing_or_handoff_refs','execution_related_flags_present','runtime_open_fields_detected','hidden_action_fields_detected','implicit_runtime_activation_fields_detected']
recommended_next_phase_step={'phase':'phase36_4_governance_archive_and_change_control_v1','goal_ru':'Закрепить архивирование governance-пакетов и change-control без runtime activation.','runtime_authorization_change':False}
marker=f"KV_PHASE36_FINAL_OPERATOR_SIGNOFF_PACKET_V1|status={status}|reason={reason}"
payload={'version':'phase36_final_operator_signoff_packet_v1','generated_at':now,'status':status,'reason':reason,'marker':marker,'signoff_packet_status':signoff_packet_status,'signoff_scope':signoff_scope,'signoff_readiness_state':signoff_readiness_state,'signoff_review_checklist':signoff_review_checklist,'required_markers':required_markers,'operator_acknowledgement_rules':operator_acknowledgement_rules,'signoff_invariants':signoff_invariants,'validation_rules':validation_rules,'rejection_rules':rejection_rules,'non_execution_confirmation':{'execution_authorized':False,'graph_write_authorized':False,'remediation_authorized':False,'runtime_phase_open':False,'signoff_packet_is_not_runtime_activation_or_execution_permission':True},'recommended_next_phase_step':recommended_next_phase_step}
out_json.write_text(json.dumps(payload,ensure_ascii=False,indent=2)+'\n')
md=['# Фаза 36.3 — Final Operator Signoff Packet v1','',f'Сформировано: {now}','',f'Маркер: `{marker}`','',f'- Статус: **{status}**',f'- Причина: **{reason}**','- Документ только для final operator signoff в design/control контуре.','- Runtime activation/execution остаются запрещены.','', '## signoff_packet_status']
for k,v in signoff_packet_status.items(): md.append(f'- {k}: {v}')
for name,section in [('signoff_scope',signoff_scope),('signoff_readiness_state',signoff_readiness_state),('required_markers',required_markers),('recommended_next_phase_step',recommended_next_phase_step)]:
 md += ['',f'## {name}']
 for k,v in section.items(): md.append(f'- {k}: {v}')
md += ['', '## signoff_review_checklist']
for i in signoff_review_checklist: md.append(f'- {i}')
md += ['', '## operator_acknowledgement_rules']
for i in operator_acknowledgement_rules: md.append(f'- {i}')
for name,vals in [('signoff_invariants',signoff_invariants),('validation_rules',validation_rules),('rejection_rules',rejection_rules)]:
 md += ['',f'## {name}']
...
print('Готово: сформирован final operator signoff packet в read-only режиме.')
