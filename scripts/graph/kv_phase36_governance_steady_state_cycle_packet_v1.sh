...
OUT_JSON=phase36_governance_steady_state_cycle_packet_v1.json \
OUT_MD=phase36_governance_steady_state_cycle_packet_v1.md
 do
...
if miss or errs or missm: status,reason='steady_state_cycle_blocked','safe_steady_state_cycle_reference_missing'
elif blocked or notes: status,reason='steady_state_cycle_ready_with_notes','safe_steady_state_cycle_reference_ready_with_notes'
else: status,reason='steady_state_cycle_ready','safe_steady_state_cycle_reference_ready'
steady_state_cycle_status={'status':status,'reason':reason,'missing_required_inputs':sorted(set(miss)),'missing_required_markers':sorted(missm),'parse_errors':errs,'triage_artifact_present':triage_present,'operator_message_ru':'Сформирован governance steady-state cycle packet в reference-only режиме.'}
steady_state_cycle_scope={'scope_target':'steady_state_cycle_of_design_control_governance_chain','governance_artifact_type':'steady_state_cycle_reference_packet','is_runtime_authorization':False,'is_execution_permit':False,'opens_implicit_runtime_transition':False,'replaces_future_runtime_phase':False,'scope_ru':'Пакет относится только к steady-state cycle design/control governance-контура.','governance_reference_only_ru':'Пакет является governance/reference артефактом.'}
steady_state_cycle_summary=['steady-state watch cycle active','watch, handoff и acknowledgement cycle consolidated','steady-state monitoring continuity preserved','operator-visible cycle summary required','no cycle summary may imply runtime readiness']
steady_state_cycle_guardrails=['governance remains monitoring/reference-only','no runtime activation','no runtime execution','no graph mutation','no remediation','no reinterpretation of cycle as operational readiness','no hidden side effects','no policy bypass','no baseline bypass','no approval bypass','no audit bypass','no steady-state-cycle-to-runtime shortcut']
steady_state_cycle_interpretation_rules=['watch remains observation-only','handoff remains governance continuity transfer','acknowledgement remains governance-only acceptance','cycle remains governance-only stability loop','no note/status/marker may be reinterpreted as runtime authorization','no cycle drift into implicit activation meaning']
steady_state_cycle_invariants=['steady-state-cycle-only governance flow','cycle-only interpretation','no runtime activation','no runtime execution','no graph mutation','no remediation','no hidden side effects','no policy bypass','no baseline bypass','no approval bypass','no audit bypass','no cycle-to-runtime shortcut','no silent execution fallback']
validation_rules=['steady_state_cycle_packet_has_required_sections','all_required_markers_present','steady_state_cycle_summary_is_complete_and_consistent','steady_state_cycle_guardrails_are_complete_and_consistent','steady_state_cycle_interpretation_rules_are_complete_and_consistent','execution_related_flags_absent','runtime_open_flags_absent','steady_state_cycle_packet_is_compatible_with_design_control_only_state']
rejection_rules=['missing_required_sections','missing_required_markers','malformed_steady_state_cycle_summary','malformed_steady_state_cycle_guardrails','malformed_steady_state_cycle_interpretation_rules','stale_steady_state_watch_handoff_ack_refs','execution_related_flags_present','runtime_open_fields_detected','hidden_action_fields_detected','implicit_runtime_activation_fields_detected']
non_execution_confirmation={'execution_authorized':False,'graph_write_authorized':False,'remediation_authorized':False,'runtime_phase_open':False,'steady_state_cycle_packet_is_not_runtime_activation_or_execution_permission':True}
recommended_next_phase_step={'phase':'phase36_21_governance_cycle_review_packet_v1','goal_ru':'Подготовить цикл review для steady-state governance-контура без открытия runtime.','runtime_authorization_change':False}
marker=f"KV_PHASE36_GOVERNANCE_STEADY_STATE_CYCLE_PACKET_V1|status={status}|reason={reason}"
payload={'version':'phase36_governance_steady_state_cycle_packet_v1','generated_at':now,'status':status,'reason':reason,'marker':marker,'steady_state_cycle_status':steady_state_cycle_status,'steady_state_cycle_scope':steady_state_cycle_scope,'steady_state_cycle_summary':steady_state_cycle_summary,'steady_state_cycle_guardrails':steady_state_cycle_guardrails,'required_markers':markers,'steady_state_cycle_interpretation_rules':steady_state_cycle_interpretation_rules,'steady_state_cycle_invariants':steady_state_cycle_invariants,'validation_rules':validation_rules,'rejection_rules':rejection_rules,'non_execution_confirmation':non_execution_confirmation,'recommended_next_phase_step':recommended_next_phase_step}
outj.write_text(json.dumps(payload,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
md=['# Фаза 36.20 — Пакет governance steady-state cycle (v1)','',f'Сформировано: {now}','',f'Маркер: `{marker}`','',f'- Статус: **{status}**',f'- Причина: **{reason}**','- Пакет фиксирует повторяемый steady-state governance cycle без открытия runtime.','','## Статус steady-state cycle (`steady_state_cycle_status`)']
for k,v in steady_state_cycle_status.items(): md.append(f'- `{k}`: {v}')
for title,obj in [('Область steady-state cycle (`steady_state_cycle_scope`)',steady_state_cycle_scope),('Обязательные маркеры (`required_markers`)',markers),('Рекомендуемый следующий шаг (`recommended_next_phase_step`)',recommended_next_phase_step)]:
    md.extend(['',f'## {title}'])
    for k,v in obj.items(): md.append(f'- `{k}`: {v}')
for title,vals in [('Сводка steady-state cycle (`steady_state_cycle_summary`)',steady_state_cycle_summary),('Guardrails steady-state cycle (`steady_state_cycle_guardrails`)',steady_state_cycle_guardrails),('Правила интерпретации цикла (`steady_state_cycle_interpretation_rules`)',steady_state_cycle_interpretation_rules),('Инварианты цикла (`steady_state_cycle_invariants`)',steady_state_cycle_invariants),('Правила валидации (`validation_rules`)',validation_rules),('Правила отклонения (`rejection_rules`)',rejection_rules)]:
    md.extend(['',f'## {title}'])
...
outm.write_text('\n'.join(md)+'\n',encoding='utf-8')
print('Готово: сформирован governance steady-state cycle packet в режиме read-only/design-only.')
