...
OUT_JSON=phase36_governance_steady_state_watch_handoff_packet_v1.json \
OUT_MD=phase36_governance_steady_state_watch_handoff_packet_v1.md
 do
...
if miss or errs or missm: status,reason='steady_state_watch_handoff_blocked','safe_steady_state_watch_handoff_reference_missing'
elif blocked or notes: status,reason='steady_state_watch_handoff_ready_with_notes','safe_steady_state_watch_handoff_reference_ready_with_notes'
else: status,reason='steady_state_watch_handoff_ready','safe_steady_state_watch_handoff_reference_ready'
steady_state_watch_handoff_status={'status':status,'reason':reason,'missing_required_inputs':sorted(set(miss)),'missing_required_markers':sorted(missm),'parse_errors':errs,'triage_artifact_present':triage_present,'operator_message_ru':'Сформирован governance steady-state watch handoff packet в reference-only режиме.'}
steady_state_watch_handoff_scope={'scope_target':'handoff_of_steady_state_watch_chain','governance_artifact_type':'steady_state_watch_handoff_reference_packet','is_runtime_authorization':False,'is_execution_permit':False,'opens_implicit_runtime_transition':False,'replaces_future_runtime_phase':False,'scope_ru':'Пакет относится только к handoff steady-state watch-контура.','governance_reference_only_ru':'Пакет является governance/reference артефактом.'}
handoff_watch_summary=['current steady-state watch scope transferred','watch targets transferred','watch signal rules transferred','watch traceability expectations transferred','operator-visible handoff summary required','no handoff summary may imply runtime readiness']
handoff_watch_continuity_rules=['watch target continuity preserved','signal interpretation continuity preserved','non-execution drift checks preserved','retained lineage watch continuity preserved','steady-state interpretation continuity preserved','no handoff may imply runtime authorization','no handoff drift into operational meaning']
handoff_watch_traceability_rules=['handoff linked to steady-state memo','handoff linked to steady-state watch packet','marker transitions traceable','retained lineage traceable','operator-visible watch-handoff summaries required','no silent handoff replacement','no hidden governance fork via handoff','no implicit runtime meaning through handoff state']
handoff_watch_invariants=['steady-state-watch-handoff-only governance flow','handoff-only interpretation','no runtime activation','no runtime execution','no graph mutation','no remediation','no hidden side effects','no policy bypass','no baseline bypass','no approval bypass','no audit bypass','no handoff-to-runtime shortcut','no silent execution fallback']
validation_rules=['steady_state_watch_handoff_packet_has_required_sections','all_required_markers_present','handoff_watch_summary_is_complete_and_consistent','handoff_watch_continuity_rules_are_complete_and_consistent','handoff_watch_traceability_rules_are_complete_and_consistent','execution_related_flags_absent','runtime_open_flags_absent','steady_state_watch_handoff_packet_is_compatible_with_design_control_only_state']
rejection_rules=['missing_required_sections','missing_required_markers','malformed_handoff_watch_summary','malformed_handoff_watch_continuity_rules','malformed_handoff_watch_traceability_rules','stale_steady_state_watch_refs','execution_related_flags_present','runtime_open_fields_detected','hidden_action_fields_detected','implicit_runtime_activation_fields_detected']
non_execution_confirmation={'execution_authorized':False,'graph_write_authorized':False,'remediation_authorized':False,'runtime_phase_open':False,'steady_state_watch_handoff_packet_is_not_runtime_activation_or_execution_permission':True}
recommended_next_phase_step={'phase':'phase36_19_governance_steady_state_watch_ack_packet_v1','goal_ru':'Подготовить подтверждение приёма steady-state watch handoff без открытия runtime.','runtime_authorization_change':False}
marker=f"KV_PHASE36_GOVERNANCE_STEADY_STATE_WATCH_HANDOFF_PACKET_V1|status={status}|reason={reason}"
payload={'version':'phase36_governance_steady_state_watch_handoff_packet_v1','generated_at':now,'status':status,'reason':reason,'marker':marker,'steady_state_watch_handoff_status':steady_state_watch_handoff_status,'steady_state_watch_handoff_scope':steady_state_watch_handoff_scope,'handoff_watch_summary':handoff_watch_summary,'handoff_watch_continuity_rules':handoff_watch_continuity_rules,'required_markers':markers,'handoff_watch_traceability_rules':handoff_watch_traceability_rules,'handoff_watch_invariants':handoff_watch_invariants,'validation_rules':validation_rules,'rejection_rules':rejection_rules,'non_execution_confirmation':non_execution_confirmation,'recommended_next_phase_step':recommended_next_phase_step}
outj.write_text(json.dumps(payload,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
md=['# Фаза 36.18 — Пакет governance steady-state watch handoff (v1)','',f'Сформировано: {now}','',f'Маркер: `{marker}`','',f'- Статус: **{status}**',f'- Причина: **{reason}**','- Пакет задаёт handoff-рамку steady-state watch-контура без открытия runtime.','','## Статус steady-state watch handoff (`steady_state_watch_handoff_status`)']
for k,v in steady_state_watch_handoff_status.items(): md.append(f'- `{k}`: {v}')
for title,obj in [('Область steady-state watch handoff (`steady_state_watch_handoff_scope`)',steady_state_watch_handoff_scope),('Обязательные маркеры (`required_markers`)',markers),('Рекомендуемый следующий шаг (`recommended_next_phase_step`)',recommended_next_phase_step)]:
    md.extend(['',f'## {title}'])
    for k,v in obj.items(): md.append(f'- `{k}`: {v}')
for title,vals in [('Сводка watch handoff (`handoff_watch_summary`)',handoff_watch_summary),('Правила continuity watch handoff (`handoff_watch_continuity_rules`)',handoff_watch_continuity_rules),('Правила traceability watch handoff (`handoff_watch_traceability_rules`)',handoff_watch_traceability_rules),('Инварианты watch handoff (`handoff_watch_invariants`)',handoff_watch_invariants),('Правила валидации (`validation_rules`)',validation_rules),('Правила отклонения (`rejection_rules`)',rejection_rules)]:
    md.extend(['',f'## {title}'])
...
outm.write_text('\n'.join(md)+'\n',encoding='utf-8')
print('Готово: сформирован governance steady-state watch handoff packet в режиме read-only/design-only.')
