...
OUT_JSON=phase36_governance_cycle_attestation_acceptance_registry_packet_v1.json \
OUT_MD=phase36_governance_cycle_attestation_acceptance_registry_packet_v1.md
 do
...
if missing_inputs or parse_errors or missing_markers:
    status = 'cycle_attestation_acceptance_registry_blocked'
    reason = 'safe_cycle_attestation_acceptance_registry_reference_missing'
elif has_blocked or has_notes:
    status = 'cycle_attestation_acceptance_registry_ready_with_notes'
    reason = 'safe_cycle_attestation_acceptance_registry_reference_ready_with_notes'
else:
    status = 'cycle_attestation_acceptance_registry_ready'
    reason = 'safe_cycle_attestation_acceptance_registry_reference_ready'

cycle_attestation_acceptance_registry_status = {
    'status': status,
    'reason': reason,
    ...
    'operator_message_ru': 'Сформирован governance cycle attestation acceptance registry packet в reference-only режиме.',
}

cycle_attestation_acceptance_registry_scope = {
    'scope_target': 'registry_of_accepted_handed_over_archived_attested_steady_state_governance_cycle',
    'governance_artifact_type': 'cycle_attestation_acceptance_registry_reference_packet',
    ...
    'scope_ru': 'Пакет относится только к registry accepted handed-over archived attest-нутого steady-state governance cycle.',
...
cycle_attestation_acceptance_registry_summary = [
    'cycle review completed',
    'cycle closure attestation archived, handed over, accepted and registered',
    'registry preserves archived cycle guardrails',
    'registry preserves archived cycle interpretation',
    'operator-visible registry summary required',
    'no registry summary may imply runtime readiness',
]

cycle_attestation_acceptance_registry_guardrails = [
    'no runtime activation',
    'no runtime execution',
    'no graph mutation',
    'no remediation',
    'no reinterpretation of registry as operational approval',
    'no hidden side effects',
    'no policy bypass',
    'no baseline bypass',
    'no approval bypass',
    'no audit bypass',
    'no registry-to-runtime shortcut',
]

cycle_attestation_acceptance_registry_traceability_rules = [
    'registry linked to cycle attestation acceptance packet',
    'registry linked to cycle attestation handover packet',
    'registry linked to cycle attestation archive packet',
    'registry linked to cycle closure attestation packet',
    'registry linked to cycle review packet',
    'registry linked to steady-state cycle packet',
    'registry linked to watch/handoff/ack chain',
    'marker transitions traceable',
    'retained lineage traceable',
    'operator-visible registry summaries required',
    'no silent registry replacement',
    'no hidden governance fork via registry',
    'no implicit runtime meaning through registry outputs',
]

cycle_attestation_acceptance_registry_invariants = [
    'cycle-attestation-acceptance-registry-only governance flow',
    'registry-only interpretation',
    'no runtime activation',
    'no runtime execution',
    'no graph mutation',
    'no remediation',
...
validation_rules = [
    'cycle_attestation_acceptance_registry_packet_has_required_sections',
    'all_required_markers_present',
    'cycle_attestation_acceptance_registry_summary_is_complete_and_consistent',
    'cycle_attestation_acceptance_registry_guardrails_are_complete_and_consistent',
    'cycle_attestation_acceptance_registry_traceability_rules_are_complete_and_consistent',
    'execution_related_flags_absent',
    'runtime_open_flags_absent',
    'cycle_attestation_acceptance_registry_packet_is_compatible_with_design_control_only_state',
]

rejection_rules = [
    'missing_required_sections',
    'missing_required_markers',
    'malformed_cycle_attestation_acceptance_registry_summary',
    'malformed_cycle_attestation_acceptance_registry_guardrails',
    'malformed_cycle_attestation_acceptance_registry_traceability_rules',
    'stale_acceptance_handover_archive_attestation_refs',
...
non_execution_confirmation = {
    ...
    'cycle_attestation_acceptance_registry_packet_is_not_runtime_activation_or_execution_permission': True,
}

recommended_next_phase_step = {
    'phase': 'phase36_27_governance_cycle_attestation_acceptance_registry_handoff_packet_v1',
    'goal_ru': 'Подготовить governance-only handoff реестра acceptance registry без открытия runtime.',
    'runtime_authorization_change': False,
}

marker = f"KV_PHASE36_GOVERNANCE_CYCLE_ATTESTATION_ACCEPTANCE_REGISTRY_PACKET_V1|status={status}|reason={reason}"
payload = {
    'version': 'phase36_governance_cycle_attestation_acceptance_registry_packet_v1',
    'generated_at': now,
    'status': status,
    'reason': reason,
    'marker': marker,
    'cycle_attestation_acceptance_registry_status': cycle_attestation_acceptance_registry_status,
    'cycle_attestation_acceptance_registry_scope': cycle_attestation_acceptance_registry_scope,
    'cycle_attestation_acceptance_registry_summary': cycle_attestation_acceptance_registry_summary,
    'cycle_attestation_acceptance_registry_guardrails': cycle_attestation_acceptance_registry_guardrails,
    'required_markers': required_markers,
    'cycle_attestation_acceptance_registry_traceability_rules': cycle_attestation_acceptance_registry_traceability_rules,
    'cycle_attestation_acceptance_registry_invariants': cycle_attestation_acceptance_registry_invariants,
...
md = [
    '# Фаза 36.26 — Пакет governance cycle attestation acceptance registry (v1)',
    '',
    f'Сформировано: {now}',
    '',
    f'Маркер: `{marker}`',
    ...
    f'- Статус: **{status}**',
    f'- Причина: **{reason}**',
    '- Пакет формализует registry accepted handed-over archived attest-нутого steady-state governance cycle как governance-only reference без открытия runtime.',
    '',
    '## Статус cycle attestation acceptance registry (`cycle_attestation_acceptance_registry_status`)',
]
for k, v in cycle_attestation_acceptance_registry_status.items():
    md.append(f'- `{k}`: {v}')

for title, obj in [
    ('Область cycle attestation acceptance registry (`cycle_attestation_acceptance_registry_scope`)', cycle_attestation_acceptance_registry_scope),
...
for title, values in [
    ('Сводка cycle attestation acceptance registry (`cycle_attestation_acceptance_registry_summary`)', cycle_attestation_acceptance_registry_summary),
    ('Guardrails cycle attestation acceptance registry (`cycle_attestation_acceptance_registry_guardrails`)', cycle_attestation_acceptance_registry_guardrails),
    ('Правила traceability cycle attestation acceptance registry (`cycle_attestation_acceptance_registry_traceability_rules`)', cycle_attestation_acceptance_registry_traceability_rules),
    ('Инварианты cycle attestation acceptance registry (`cycle_attestation_acceptance_registry_invariants`)', cycle_attestation_acceptance_registry_invariants),
...
outm.write_text('\n'.join(md) + '\n', encoding='utf-8')
print('Готово: сформирован governance cycle attestation acceptance registry packet в режиме read-only/design-only.')
print('Активация runtime, исполнение, изменения графа и remediation остаются закрытыми.')
