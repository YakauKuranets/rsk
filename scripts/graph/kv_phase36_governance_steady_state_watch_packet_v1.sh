...
OUT_JSON=phase36_governance_steady_state_watch_packet_v1.json \
OUT_MD=phase36_governance_steady_state_watch_packet_v1.md
 do
...
if miss or errs or missm: status,reason='steady_state_watch_blocked','safe_steady_state_watch_reference_missing'
elif blocked or notes: status,reason='steady_state_watch_ready_with_notes','safe_steady_state_watch_reference_ready_with_notes'
else: status,reason='steady_state_watch_ready','safe_steady_state_watch_reference_ready'
steady_state_watch_status={'status':status,'reason':reason,'missing_required_inputs':sorted(set(miss)),'missing_required_markers':sorted(missm),'parse_errors':errs,'triage_artifact_present':triage_present,'operator_message_ru':'Сформирован governance steady-state watch packet в reference-only режиме.'}
steady_state_watch_scope={'scope_target':'watch_mode_for_steady_state_governance_chain','governance_artifact_type':'steady_state_watch_reference_packet','is_runtime_authorization':False,'is_execution_permit':False,'opens_implicit_runtime_transition':False,'replaces_future_runtime_phase':False,'scope_ru':'Пакет относится только к watch-режиму steady-state governance-контура.','governance_reference_only_ru':'Пакет является governance/reference артефактом.'}
watch_targets=['steady-state summary stability','steady-state guardrails stability','interpretation rules stability','non-execution boundary stability','retained-record and retention-audit continuity','no drift toward runtime-like meaning','operator-visible watch summary required']
watch_signal_rules=['steady-state drift detection','guardrail weakening detection','interpretation drift detection','non-execution flag drift detection','retained lineage drift detection','no watch signal may imply runtime authorization','no watch signal may open runtime path','notes escalation if steady-state governance meaning weakens']
watch_traceability_rules=['watch state linked to steady-state memo','watch state linked to closure and monitoring chain','marker transitions traceable','retained lineage traceable','operator-visible watch summaries required','no silent watch-state replacement','no hidden governance fork through watch layer','no implicit runtime meaning through watch outputs']
watch_invariants=['steady-state-watch-only governance flow','watch-only interpretation','no runtime activation','no runtime execution','no graph mutation','no remediation','no hidden side effects','no policy bypass','no baseline bypass','no approval bypass','no audit bypass','no watch-to-runtime shortcut','no silent execution fallback']
validation_rules=['steady_state_watch_packet_has_required_sections','all_required_markers_present','watch_targets_are_complete_and_consistent','watch_signal_rules_are_complete_and_consistent','watch_traceability_rules_are_complete_and_consistent','execution_related_flags_absent','runtime_open_flags_absent','steady_state_watch_packet_is_compatible_with_design_control_only_state']
rejection_rules=['missing_required_sections','missing_required_markers','malformed_watch_targets','malformed_watch_signal_rules','malformed_watch_traceability_rules','stale_steady_state_monitoring_refs','execution_related_flags_present','runtime_open_fields_detected','hidden_action_fields_detected','implicit_runtime_activation_fields_detected']
non_execution_confirmation={'execution_authorized':False,'graph_write_authorized':False,'remediation_authorized':False,'runtime_phase_open':False,'steady_state_watch_packet_is_not_runtime_activation_or_execution_permission':True}
recommended_next_phase_step={'phase':'phase36_18_governance_steady_state_watch_handoff_packet_v1','goal_ru':'Подготовить handoff-пакет watch-режима без открытия runtime.','runtime_authorization_change':False}
marker=f"KV_PHASE36_GOVERNANCE_STEADY_STATE_WATCH_PACKET_V1|status={status}|reason={reason}"
payload={'version':'phase36_governance_steady_state_watch_packet_v1','generated_at':now,'status':status,'reason':reason,'marker':marker,'steady_state_watch_status':steady_state_watch_status,'steady_state_watch_scope':steady_state_watch_scope,'watch_targets':watch_targets,'watch_signal_rules':watch_signal_rules,'required_markers':markers,'watch_traceability_rules':watch_traceability_rules,'watch_invariants':watch_invariants,'validation_rules':validation_rules,'rejection_rules':rejection_rules,'non_execution_confirmation':non_execution_confirmation,'recommended_next_phase_step':recommended_next_phase_step}
outj.write_text(json.dumps(payload,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
md=['# Фаза 36.17 — Пакет governance steady-state watch (v1)','',f'Сформировано: {now}','',f'Маркер: `{marker}`','',f'- Статус: **{status}**',f'- Причина: **{reason}**','- Пакет задаёт watch-режим устойчивого governance-контура без открытия runtime.','','## Статус steady-state watch (`steady_state_watch_status`)']
for k,v in steady_state_watch_status.items(): md.append(f'- `{k}`: {v}')
for title,obj in [('Область steady-state watch (`steady_state_watch_scope`)',steady_state_watch_scope),('Обязательные маркеры (`required_markers`)',markers),('Рекомендуемый следующий шаг (`recommended_next_phase_step`)',recommended_next_phase_step)]:
    md.extend(['',f'## {title}'])
    for k,v in obj.items(): md.append(f'- `{k}`: {v}')
for title,vals in [('Цели watch (`watch_targets`)',watch_targets),('Правила watch-сигналов (`watch_signal_rules`)',watch_signal_rules),('Правила traceability watch (`watch_traceability_rules`)',watch_traceability_rules),('Инварианты watch (`watch_invariants`)',watch_invariants),('Правила валидации (`validation_rules`)',validation_rules),('Правила отклонения (`rejection_rules`)',rejection_rules)]:
    md.extend(['',f'## {title}'])
...
outm.write_text('\n'.join(md)+'\n',encoding='utf-8')
print('Готово: сформирован governance steady-state watch packet в режиме read-only/design-only.')
