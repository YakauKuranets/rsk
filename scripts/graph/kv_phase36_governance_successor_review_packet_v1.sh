...
export OUT_JSON="${ROOT_DIR}/docs/phase36_governance_successor_review_packet_v1.json"
export OUT_MD="${ROOT_DIR}/docs/phase36_governance_successor_review_packet_v1.md"
...
if missing_inputs or parse_errors or missing_markers:
    status = "successor_review_blocked"
    reason = "safe_successor_review_reference_missing"
elif has_blocked or has_notes:
    status = "successor_review_ready_with_notes"
    reason = "safe_successor_review_reference_ready_with_notes"
else:
    status = "successor_review_ready"
    reason = "safe_successor_review_reference_ready"
...
successor_review_status = {
    ...
    "operator_message_ru": "Сформирован governance successor review packet в reference-only режиме.",
}

successor_review_scope = {
    "scope_target": "review_first_versioned_governance_successor_packet",
    "governance_artifact_type": "successor_review_reference_packet",
    ...
    "scope_ru": "Пакет относится только к review первого versioned governance successor packet.",
    "governance_reference_only_ru": "Пакет является governance/reference артефактом review-only контура.",
}

review_criteria = [
    "successor packet matches template schema",
    "predecessor linkage verified",
    "change-control linkage verified",
    "maintenance linkage verified",
    "governance chain refs complete",
    "marker set complete",
    "continuity summary valid",
    "traceability summary valid",
    "operator notes allowed but not permission-granting",
    "no runtime-opening semantics present",
]

continuity_review_rules = [
    "policy continuity review required",
    "baseline continuity review required",
    "approval continuity review required",
    "audit continuity review required",
    "governance continuity review required",
    "archive/change-control continuity review required",
    "maintenance continuity review required",
    "no signoff reuse as runtime permission",
    "no continuity shortcut into runtime state",
]

traceability_review_rules = [
    "predecessor traceability required",
    "upstream refs traceability required",
    "marker transition traceability required",
    "version increment traceability required",
    "archived chain preservation review required",
    "no silent successor replacement",
    "no hidden governance fork",
    "operator-visible review summary required",
]

review_invariants = [
    "review-only governance flow",
    "successor-review-only interpretation",
    ...
    "no approval bypass",
    "no audit bypass",
    "no template-to-runtime shortcut",
    "no silent execution fallback",
]

validation_rules = [
    "successor_review_packet_has_required_sections",
    "all_required_markers_present",
    "review_criteria_are_complete_and_consistent",
    "continuity_review_rules_are_complete_and_consistent",
    "traceability_review_rules_are_complete_and_consistent",
    "execution_related_flags_absent",
    "runtime_open_flags_absent",
    "successor_review_packet_is_compatible_with_design_control_only_state",
]

rejection_rules = [
    "missing_required_sections",
    "missing_required_markers",
    "malformed_review_criteria",
    "malformed_continuity_review_rules",
    "malformed_traceability_review_rules",
    "stale_governance_template_refs",
...
non_execution_confirmation = {
    ...
    "successor_review_packet_is_not_runtime_activation_or_execution_permission": True,
}

recommended_next_phase_step = {
    "phase": "phase36_8_successor_review_checklist_runbook_v1",
    "goal_ru": "Подготовить runbook применения review-критериев к первому successor packet без открытия runtime.",
    "runtime_authorization_change": False,
}

marker = f"KV_PHASE36_GOVERNANCE_SUCCESSOR_REVIEW_PACKET_V1|status={status}|reason={reason}"

payload = {
    "version": "phase36_governance_successor_review_packet_v1",
    "generated_at": now,
    "status": status,
    "reason": reason,
    "marker": marker,
    "successor_review_status": successor_review_status,
    "successor_review_scope": successor_review_scope,
    "review_criteria": review_criteria,
    "continuity_review_rules": continuity_review_rules,
    "required_markers": required_markers,
    "traceability_review_rules": traceability_review_rules,
...
md = [
    "# Фаза 36.7 — Governance Successor Review Packet v1",
    "",
    f"Сформировано: {now}",
    "",
    f"Маркер: `{marker}`",
    ...
    f"- Статус: **{status}**",
    f"- Причина: **{reason}**",
    "- Пакет фиксирует review-критерии для первого versioned governance successor без открытия runtime.",
    "",
    "## successor_review_status",
]
for k, v in successor_review_status.items():
    md.append(f"- {k}: {v}")
for name, obj in [
    ("successor_review_scope", successor_review_scope),
    ("required_markers", required_markers),
...
print("Готово: сформирован governance successor review packet в режиме read-only/design-only.")
