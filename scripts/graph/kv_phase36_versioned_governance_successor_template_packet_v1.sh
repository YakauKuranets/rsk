...
export OUT_JSON="${ROOT_DIR}/docs/phase36_versioned_governance_successor_template_packet_v1.json"
export OUT_MD="${ROOT_DIR}/docs/phase36_versioned_governance_successor_template_packet_v1.md"
...
if missing_inputs or parse_errors or missing_markers:
    status = "successor_template_blocked"
    reason = "safe_successor_reference_missing"
elif has_blocked or has_notes:
    status = "successor_template_ready_with_notes"
    reason = "safe_successor_reference_ready_with_notes"
else:
    status = "successor_template_ready"
    reason = "safe_successor_reference_ready"

successor_template_status = {
    "status": status,
    "reason": reason,
    ...
    "operator_message_ru": "Сформирован versioned governance successor template packet в reference-only режиме.",
}

successor_template_scope = {
    "scope_target": "versioned_governance_successor_template_after_maintenance_window_packet",
    "governance_artifact_type": "successor_template_reference_packet",
    ...
    "scope_ru": "Пакет относится только к successor template после maintenance window packet.",
    "governance_reference_only_ru": "Пакет является governance/reference артефактом successor-template контура.",
}

successor_packet_schema = [
    "successor_packet_id",
...
successor_continuity_requirements = [
    "predecessor linkage required",
    "policy continuity required",
    "baseline continuity required",
    "approval continuity required",
    "audit continuity required",
    "governance continuity required",
    "archive/change-control continuity required",
    "maintenance continuity required",
    "no runtime-opening continuity shortcut",
    "no signoff reuse as runtime permission",
]

successor_traceability_requirements = [
    "all upstream references traceable",
    "all marker transitions traceable",
    "version increment traceable",
    "operator-visible successor summary required",
    "archived chain preservation required",
    "no silent successor replacement",
    "no hidden governance fork",
]

successor_invariants = [
    "successor-template-only governance flow",
    "versioned-successor-only interpretation",
    ...
    "no approval bypass",
    "no audit bypass",
    "no maintenance-to-runtime shortcut",
    "no silent execution fallback",
]

validation_rules = [
    "successor_template_packet_has_required_sections",
    "all_required_markers_present",
    "successor_packet_schema_is_complete_and_consistent",
    "continuity_requirements_are_complete_and_consistent",
    "traceability_requirements_are_complete_and_consistent",
    "execution_related_flags_absent",
    "runtime_open_flags_absent",
    "successor_template_packet_is_compatible_with_design_control_only_state",
]

rejection_rules = [
    "missing_required_sections",
    "missing_required_markers",
    "malformed_successor_packet_schema",
    "malformed_continuity_requirements",
    "malformed_traceability_requirements",
    "stale_governance_archive_maintenance_refs",
...
non_execution_confirmation = {
    ...
    "successor_template_packet_is_not_runtime_activation_or_execution_permission": True,
}

recommended_next_phase_step = {
    "phase": "phase36_7_governance_successor_review_packet_v1",
    "goal_ru": "Подготовить пакет review-критериев для проверки первого versioned successor без открытия runtime.",
    "runtime_authorization_change": False,
}

marker = f"KV_PHASE36_VERSIONED_GOVERNANCE_SUCCESSOR_TEMPLATE_PACKET_V1|status={status}|reason={reason}"

payload = {
    "version": "phase36_versioned_governance_successor_template_packet_v1",
    "generated_at": now,
    "status": status,
    "reason": reason,
    "marker": marker,
    "successor_template_status": successor_template_status,
    "successor_template_scope": successor_template_scope,
    "successor_packet_schema": successor_packet_schema,
    "successor_continuity_requirements": successor_continuity_requirements,
    "required_markers": required_markers,
    "successor_traceability_requirements": successor_traceability_requirements,
    "successor_invariants": successor_invariants,
...
md = [
    "# Фаза 36.6 — Versioned Governance Successor Template Packet v1",
    "",
    f"Сформировано: {now}",
    "",
    f"Маркер: `{marker}`",
    ...
    f"- Статус: **{status}**",
    f"- Причина: **{reason}**",
    "- Пакет фиксирует канонический шаблон versioned governance successor без открытия runtime.",
    "",
    "## successor_template_status",
]
for k, v in successor_template_status.items():
    md.append(f"- {k}: {v}")
for name, obj in [
    ("successor_template_scope", successor_template_scope),
    ("required_markers", required_markers),
...
print("Готово: сформирован versioned governance successor template packet в режиме read-only/design-only.")
