...
OUT_JSON=phase36_governance_cycle_closure_attestation_packet_v1.json \
OUT_MD=phase36_governance_cycle_closure_attestation_packet_v1.md
 do
...
if missing_inputs or parse_errors or missing_markers:
    status = 'cycle_closure_attestation_blocked'
    reason = 'safe_cycle_closure_attestation_reference_missing'
elif has_blocked or has_notes:
    status = 'cycle_closure_attestation_ready_with_notes'
    reason = 'safe_cycle_closure_attestation_reference_ready_with_notes'
else:
    status = 'cycle_closure_attestation_ready'
    reason = 'safe_cycle_closure_attestation_reference_ready'

cycle_closure_attestation_status = {
    'status': status,
    'reason': reason,
    ...
    'operator_message_ru': 'Сформирован governance cycle closure attestation packet в reference-only режиме.',
}

cycle_closure_attestation_scope = {
    'scope_target': 'closure_attestation_of_full_steady_state_governance_cycle',
    'governance_artifact_type': 'cycle_closure_attestation_reference_packet',
    ...
    'scope_ru': 'Пакет относится только к closure attestation полного steady-state governance cycle.',
    'governance_reference_only_ru': 'Пакет является governance/reference артефактом.',
}

cycle_closure_attestation_summary = [
    'cycle review completed',
    'cycle closure attested',
    'cycle guardrails preserved',
    'cycle interpretation preserved',
    'operator-visible attestation summary required',
    'no attestation summary may imply runtime readiness',
]

cycle_closure_attestation_guardrails = [
    'no runtime activation',
    'no runtime execution',
    'no graph mutation',
    'no remediation',
    'no reinterpretation of attestation as operational approval',
    'no hidden side effects',
    'no policy bypass',
    'no baseline bypass',
    'no approval bypass',
    'no audit bypass',
    'no attestation-to-runtime shortcut',
]

cycle_closure_attestation_traceability_rules = [
    'attestation linked to cycle review packet',
    'attestation linked to steady-state cycle packet',
    'attestation linked to watch/handoff/ack chain',
    'marker transitions traceable',
    'retained lineage traceable',
    'operator-visible attestation summaries required',
    'no silent attestation replacement',
    'no hidden governance fork via attestation',
    'no implicit runtime meaning through attestation outputs',
]

cycle_closure_attestation_invariants = [
    'cycle-closure-attestation-only governance flow',
    'attestation-only interpretation',
    'no runtime activation',
    'no runtime execution',
    'no graph mutation',
    'no remediation',
...
validation_rules = [
    'cycle_closure_attestation_packet_has_required_sections',
    'all_required_markers_present',
    'cycle_closure_attestation_summary_is_complete_and_consistent',
    'cycle_closure_attestation_guardrails_are_complete_and_consistent',
    'cycle_closure_attestation_traceability_rules_are_complete_and_consistent',
    'execution_related_flags_absent',
    'runtime_open_flags_absent',
    'cycle_closure_attestation_packet_is_compatible_with_design_control_only_state',
]

rejection_rules = [
    'missing_required_sections',
    'missing_required_markers',
    'malformed_cycle_closure_attestation_summary',
    'malformed_cycle_closure_attestation_guardrails',
    'malformed_cycle_closure_attestation_traceability_rules',
    'stale_cycle_review_steady_state_refs',
...
non_execution_confirmation = {
    ...
    'cycle_closure_attestation_packet_is_not_runtime_activation_or_execution_permission': True,
}

recommended_next_phase_step = {
    'phase': 'phase36_23_governance_cycle_attestation_archive_packet_v1',
    'goal_ru': 'Подготовить архивный attestation-пакет governance-цикла без открытия runtime.',
    'runtime_authorization_change': False,
}

marker = f"KV_PHASE36_GOVERNANCE_CYCLE_CLOSURE_ATTESTATION_PACKET_V1|status={status}|reason={reason}"
payload = {
    'version': 'phase36_governance_cycle_closure_attestation_packet_v1',
    'generated_at': now,
    'status': status,
    'reason': reason,
    'marker': marker,
    'cycle_closure_attestation_status': cycle_closure_attestation_status,
    'cycle_closure_attestation_scope': cycle_closure_attestation_scope,
    'cycle_closure_attestation_summary': cycle_closure_attestation_summary,
    'cycle_closure_attestation_guardrails': cycle_closure_attestation_guardrails,
    'required_markers': required_markers,
    'cycle_closure_attestation_traceability_rules': cycle_closure_attestation_traceability_rules,
    'cycle_closure_attestation_invariants': cycle_closure_attestation_invariants,
...
md = [
    '# Фаза 36.22 — Пакет governance cycle closure attestation (v1)',
    '',
    f'Сформировано: {now}',
    '',
    f'Маркер: `{marker}`',
    ...
    f'- Статус: **{status}**',
    f'- Причина: **{reason}**',
    '- Пакет фиксирует closure attestation полного steady-state governance cycle без открытия runtime.',
    '',
    '## Статус cycle closure attestation (`cycle_closure_attestation_status`)',
]
for k, v in cycle_closure_attestation_status.items():
    md.append(f'- `{k}`: {v}')

for title, obj in [
    ('Область cycle closure attestation (`cycle_closure_attestation_scope`)', cycle_closure_attestation_scope),
...
for title, values in [
    ('Сводка cycle closure attestation (`cycle_closure_attestation_summary`)', cycle_closure_attestation_summary),
    ('Guardrails cycle closure attestation (`cycle_closure_attestation_guardrails`)', cycle_closure_attestation_guardrails),
    ('Правила traceability cycle closure attestation (`cycle_closure_attestation_traceability_rules`)', cycle_closure_attestation_traceability_rules),
    ('Инварианты cycle closure attestation (`cycle_closure_attestation_invariants`)', cycle_closure_attestation_invariants),
...
outm.write_text('\n'.join(md) + '\n', encoding='utf-8')
print('Готово: сформирован governance cycle closure attestation packet в режиме read-only/design-only.')
