...
OUT_JSON=phase36_governance_cycle_review_packet_v1.json \
OUT_MD=phase36_governance_cycle_review_packet_v1.md
 do
...
if miss or errs or missm: status,reason='cycle_review_blocked','safe_cycle_review_reference_missing'
elif blocked or notes: status,reason='cycle_review_ready_with_notes','safe_cycle_review_reference_ready_with_notes'
else: status,reason='cycle_review_ready','safe_cycle_review_reference_ready'
cycle_review_status={'status':status,'reason':reason,'missing_required_inputs':sorted(set(miss)),'missing_required_markers':sorted(missm),'parse_errors':errs,'triage_artifact_present':triage_present,'operator_message_ru':'Сформирован governance cycle review packet в reference-only режиме.'}
cycle_review_scope={'scope_target':'review_of_full_steady_state_governance_cycle','governance_artifact_type':'cycle_review_reference_packet','is_runtime_authorization':False,'is_execution_permit':False,'opens_implicit_runtime_transition':False,'replaces_future_runtime_phase':False,'scope_ru':'Пакет относится только к review полного steady-state governance cycle.','governance_reference_only_ru':'Пакет является governance/reference артефактом.'}
cycle_review_criteria=['cycle summary complete and coherent','cycle guardrails complete and consistent','cycle interpretation rules complete and consistent','watch/handoff/ack linkage preserved','governance-only interpretation preserved','no review criterion may imply runtime readiness']
cycle_review_guardrails=['no runtime activation','no runtime execution','no graph mutation','no remediation','no reinterpretation of cycle review as operational approval','no hidden side effects','no policy bypass','no baseline bypass','no approval bypass','no audit bypass','no cycle-review-to-runtime shortcut']
cycle_review_traceability_rules=['cycle review linked to steady-state cycle packet','cycle review linked to watch/handoff/ack chain','marker transitions traceable','retained lineage traceable','operator-visible cycle review summaries required','no silent review replacement','no hidden governance fork via cycle review','no implicit runtime meaning through cycle review outputs']
cycle_review_invariants=['cycle-review-only governance flow','review-only interpretation','no runtime activation','no runtime execution','no graph mutation','no remediation','no hidden side effects','no policy bypass','no baseline bypass','no approval bypass','no audit bypass','no cycle-review-to-runtime shortcut','no silent execution fallback']
validation_rules=['cycle_review_packet_has_required_sections','all_required_markers_present','cycle_review_criteria_are_complete_and_consistent','cycle_review_guardrails_are_complete_and_consistent','cycle_review_traceability_rules_are_complete_and_consistent','execution_related_flags_absent','runtime_open_flags_absent','cycle_review_packet_is_compatible_with_design_control_only_state']
rejection_rules=['missing_required_sections','missing_required_markers','malformed_cycle_review_criteria','malformed_cycle_review_guardrails','malformed_cycle_review_traceability_rules','stale_steady_state_cycle_refs','execution_related_flags_present','runtime_open_fields_detected','hidden_action_fields_detected','implicit_runtime_activation_fields_detected']
non_execution_confirmation={'execution_authorized':False,'graph_write_authorized':False,'remediation_authorized':False,'runtime_phase_open':False,'cycle_review_packet_is_not_runtime_activation_or_execution_permission':True}
recommended_next_phase_step={'phase':'phase36_22_governance_cycle_signoff_packet_v1','goal_ru':'Подготовить cycle signoff packet без открытия runtime.','runtime_authorization_change':False}
marker=f"KV_PHASE36_GOVERNANCE_CYCLE_REVIEW_PACKET_V1|status={status}|reason={reason}"
payload={'version':'phase36_governance_cycle_review_packet_v1','generated_at':now,'status':status,'reason':reason,'marker':marker,'cycle_review_status':cycle_review_status,'cycle_review_scope':cycle_review_scope,'cycle_review_criteria':cycle_review_criteria,'cycle_review_guardrails':cycle_review_guardrails,'required_markers':markers,'cycle_review_traceability_rules':cycle_review_traceability_rules,'cycle_review_invariants':cycle_review_invariants,'validation_rules':validation_rules,'rejection_rules':rejection_rules,'non_execution_confirmation':non_execution_confirmation,'recommended_next_phase_step':recommended_next_phase_step}
outj.write_text(json.dumps(payload,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
md=['# Фаза 36.21 — Пакет governance cycle review (v1)','',f'Сформировано: {now}','',f'Маркер: `{marker}`','',f'- Статус: **{status}**',f'- Причина: **{reason}**','- Пакет формализует review полного steady-state governance cycle без открытия runtime.','','## Статус cycle review (`cycle_review_status`)']
for k,v in cycle_review_status.items(): md.append(f'- `{k}`: {v}')
for title,obj in [('Область cycle review (`cycle_review_scope`)',cycle_review_scope),('Обязательные маркеры (`required_markers`)',markers),('Рекомендуемый следующий шаг (`recommended_next_phase_step`)',recommended_next_phase_step)]:
    md.extend(['',f'## {title}'])
    for k,v in obj.items(): md.append(f'- `{k}`: {v}')
for title,vals in [('Критерии cycle review (`cycle_review_criteria`)',cycle_review_criteria),('Guardrails cycle review (`cycle_review_guardrails`)',cycle_review_guardrails),('Правила traceability cycle review (`cycle_review_traceability_rules`)',cycle_review_traceability_rules),('Инварианты cycle review (`cycle_review_invariants`)',cycle_review_invariants),('Правила валидации (`validation_rules`)',validation_rules),('Правила отклонения (`rejection_rules`)',rejection_rules)]:
    md.extend(['',f'## {title}'])
...
outm.write_text('\n'.join(md)+'\n',encoding='utf-8')
print('Готово: сформирован governance cycle review packet в режиме read-only/design-only.')
