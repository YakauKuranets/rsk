...
OUT_JSON=phase36_governance_cycle_attestation_archive_packet_v1.json \
OUT_MD=phase36_governance_cycle_attestation_archive_packet_v1.md
 do
...
if missing_inputs or parse_errors or missing_markers:
    status = 'cycle_attestation_archive_blocked'
    reason = 'safe_cycle_attestation_archive_reference_missing'
elif has_blocked or has_notes:
    status = 'cycle_attestation_archive_ready_with_notes'
    reason = 'safe_cycle_attestation_archive_reference_ready_with_notes'
else:
    status = 'cycle_attestation_archive_ready'
    reason = 'safe_cycle_attestation_archive_reference_ready'

cycle_attestation_archive_status = {
    'status': status,
    'reason': reason,
    ...
    'operator_message_ru': 'Сформирован governance cycle attestation archive packet в reference-only режиме.',
}

cycle_attestation_archive_scope = {
    'scope_target': 'archive_stage_of_attested_steady_state_governance_cycle',
    'governance_artifact_type': 'cycle_attestation_archive_reference_packet',
    ...
    'scope_ru': 'Пакет относится только к archive-стадии attest-нутого steady-state governance cycle.',
    'governance_reference_only_ru': 'Пакет является governance/reference артефактом.',
}

cycle_attestation_archive_summary = [
    'cycle review completed',
    'cycle closure attestation archived',
    'archived cycle guardrails preserved',
    'archived cycle interpretation preserved',
    'operator-visible archive summary required',
    'no archive summary may imply runtime readiness',
]

cycle_attestation_archive_guardrails = [
    'no runtime activation',
    'no runtime execution',
    'no graph mutation',
    'no remediation',
    'no reinterpretation of archive as operational approval',
...
cycle_attestation_archive_traceability_rules = [
    ...
    'no hidden governance fork via archive',
    'no implicit runtime meaning through archive outputs',
]

cycle_attestation_archive_invariants = [
    'cycle-attestation-archive-only governance flow',
    'archive-only interpretation',
    'no runtime activation',
    'no runtime execution',
    'no graph mutation',
    'no remediation',
...
validation_rules = [
    'cycle_attestation_archive_packet_has_required_sections',
    'all_required_markers_present',
    'cycle_attestation_archive_summary_is_complete_and_consistent',
    'cycle_attestation_archive_guardrails_are_complete_and_consistent',
    'cycle_attestation_archive_traceability_rules_are_complete_and_consistent',
    'execution_related_flags_absent',
    'runtime_open_flags_absent',
    'cycle_attestation_archive_packet_is_compatible_with_design_control_only_state',
]

rejection_rules = [
    'missing_required_sections',
    'missing_required_markers',
    'malformed_cycle_attestation_archive_summary',
    'malformed_cycle_attestation_archive_guardrails',
    'malformed_cycle_attestation_archive_traceability_rules',
    'stale_cycle_attestation_steady_state_refs',
...
non_execution_confirmation = {
    ...
    'cycle_attestation_archive_packet_is_not_runtime_activation_or_execution_permission': True,
}

recommended_next_phase_step = {
    'phase': 'phase36_24_governance_cycle_attestation_handover_packet_v1',
    'goal_ru': 'Подготовить handover-пакет архивированного attestation-цикла без открытия runtime.',
    'runtime_authorization_change': False,
}

marker = f"KV_PHASE36_GOVERNANCE_CYCLE_ATTESTATION_ARCHIVE_PACKET_V1|status={status}|reason={reason}"
payload = {
    'version': 'phase36_governance_cycle_attestation_archive_packet_v1',
    'generated_at': now,
    'status': status,
    'reason': reason,
    'marker': marker,
    'cycle_attestation_archive_status': cycle_attestation_archive_status,
    'cycle_attestation_archive_scope': cycle_attestation_archive_scope,
    'cycle_attestation_archive_summary': cycle_attestation_archive_summary,
    'cycle_attestation_archive_guardrails': cycle_attestation_archive_guardrails,
    'required_markers': required_markers,
    'cycle_attestation_archive_traceability_rules': cycle_attestation_archive_traceability_rules,
    'cycle_attestation_archive_invariants': cycle_attestation_archive_invariants,
...
md = [
    '# Фаза 36.23 — Пакет governance cycle attestation archive (v1)',
    '',
    f'Сформировано: {now}',
    '',
    f'Маркер: `{marker}`',
    ...
    f'- Статус: **{status}**',
    f'- Причина: **{reason}**',
    '- Пакет фиксирует archive-состояние attest-нутого steady-state governance cycle без открытия runtime.',
    '',
    '## Статус cycle attestation archive (`cycle_attestation_archive_status`)',
]
for k, v in cycle_attestation_archive_status.items():
    md.append(f'- `{k}`: {v}')

for title, obj in [
    ('Область cycle attestation archive (`cycle_attestation_archive_scope`)', cycle_attestation_archive_scope),
...
for title, values in [
    ('Сводка cycle attestation archive (`cycle_attestation_archive_summary`)', cycle_attestation_archive_summary),
    ('Guardrails cycle attestation archive (`cycle_attestation_archive_guardrails`)', cycle_attestation_archive_guardrails),
    ('Правила traceability cycle attestation archive (`cycle_attestation_archive_traceability_rules`)', cycle_attestation_archive_traceability_rules),
    ('Инварианты cycle attestation archive (`cycle_attestation_archive_invariants`)', cycle_attestation_archive_invariants),
...
outm.write_text('\n'.join(md) + '\n', encoding='utf-8')
print('Готово: сформирован governance cycle attestation archive packet в режиме read-only/design-only.')
