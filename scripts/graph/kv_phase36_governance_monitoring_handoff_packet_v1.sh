...
OUT_JSON=phase36_governance_monitoring_handoff_packet_v1.json \
OUT_MD=phase36_governance_monitoring_handoff_packet_v1.md
 do
...
if miss or errs or missm: status,reason='monitoring_handoff_blocked','safe_monitoring_handoff_reference_missing'
elif blocked or notes: status,reason='monitoring_handoff_ready_with_notes','safe_monitoring_handoff_reference_ready_with_notes'
else: status,reason='monitoring_handoff_ready','safe_monitoring_handoff_reference_ready'
monitoring_handoff_status={'status':status,'reason':reason,'missing_required_inputs':sorted(set(miss)),'missing_required_markers':sorted(missm),'parse_errors':errs,'triage_artifact_present':triage_present,'operator_message_ru':'Сформирован governance monitoring handoff packet в reference-only режиме.'}
monitoring_handoff_scope={'scope_target':'handoff_of_post_closure_monitoring_chain','governance_artifact_type':'monitoring_handoff_reference_packet','is_runtime_authorization':False,'is_execution_permit':False,'opens_implicit_runtime_transition':False,'replaces_future_runtime_phase':False,'scope_ru':'Пакет относится только к handoff post-closure monitoring-контура.','governance_reference_only_ru':'Пакет является governance/reference артефактом.'}
handoff_monitoring_summary=['current monitoring scope transferred','monitoring targets transferred','monitoring signal rules transferred','retained chain monitoring context transferred','closure-state monitoring context transferred','operator-visible handoff summary required','no handoff summary may imply runtime readiness']
handoff_signal_continuity_rules=['monitoring target continuity preserved','signal interpretation continuity preserved','non-execution drift checks preserved','retained lineage checks preserved','closure interpretation preserved','no signal handoff may imply runtime authorization','no handoff drift into operational meaning']
handoff_traceability_rules=['handoff linked to closure state','handoff linked to monitoring packet','marker transitions traceable','retained chain lineage traceable','monitoring summaries operator-visible','no silent handoff replacement','no hidden governance fork via handoff','no implicit runtime meaning through handoff state']
handoff_invariants=['monitoring-handoff-only governance flow','handoff-only interpretation','no runtime activation','no runtime execution','no graph mutation','no remediation','no hidden side effects','no policy bypass','no baseline bypass','no approval bypass','no audit bypass','no handoff-to-runtime shortcut','no silent execution fallback']
validation_rules=['monitoring_handoff_packet_has_required_sections','all_required_markers_present','handoff_monitoring_summary_is_complete_and_consistent','handoff_signal_continuity_rules_are_complete_and_consistent','handoff_traceability_rules_are_complete_and_consistent','execution_related_flags_absent','runtime_open_flags_absent','monitoring_handoff_packet_is_compatible_with_design_control_only_state']
rejection_rules=['missing_required_sections','missing_required_markers','malformed_handoff_monitoring_summary','malformed_handoff_signal_continuity_rules','malformed_handoff_traceability_rules','stale_closure_monitoring_refs','execution_related_flags_present','runtime_open_fields_detected','hidden_action_fields_detected','implicit_runtime_activation_fields_detected']
non_execution_confirmation={'execution_authorized':False,'graph_write_authorized':False,'remediation_authorized':False,'runtime_phase_open':False,'monitoring_handoff_packet_is_not_runtime_activation_or_execution_permission':True}
recommended_next_phase_step={'phase':'phase36_16_governance_monitoring_ack_packet_v1','goal_ru':'Подготовить подтверждение приёма monitoring handoff без открытия runtime.','runtime_authorization_change':False}
marker=f"KV_PHASE36_GOVERNANCE_MONITORING_HANDOFF_PACKET_V1|status={status}|reason={reason}"
payload={'version':'phase36_governance_monitoring_handoff_packet_v1','generated_at':now,'status':status,'reason':reason,'marker':marker,'monitoring_handoff_status':monitoring_handoff_status,'monitoring_handoff_scope':monitoring_handoff_scope,'handoff_monitoring_summary':handoff_monitoring_summary,'handoff_signal_continuity_rules':handoff_signal_continuity_rules,'required_markers':markers,'handoff_traceability_rules':handoff_traceability_rules,'handoff_invariants':handoff_invariants,'validation_rules':validation_rules,'rejection_rules':rejection_rules,'non_execution_confirmation':non_execution_confirmation,'recommended_next_phase_step':recommended_next_phase_step}
outj.write_text(json.dumps(payload,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
md=['# Фаза 36.15 — Пакет governance monitoring handoff (v1)','',f'Сформировано: {now}','',f'Маркер: `{marker}`','',f'- Статус: **{status}**',f'- Причина: **{reason}**','- Пакет задаёт handoff-рамку monitoring-контура без открытия runtime.','','## Статус monitoring handoff (`monitoring_handoff_status`)']
for k,v in monitoring_handoff_status.items(): md.append(f'- `{k}`: {v}')
for title,obj in [('Область monitoring handoff (`monitoring_handoff_scope`)',monitoring_handoff_scope),('Обязательные маркеры (`required_markers`)',markers),('Рекомендуемый следующий шаг (`recommended_next_phase_step`)',recommended_next_phase_step)]:
    md.extend(['',f'## {title}'])
    for k,v in obj.items(): md.append(f'- `{k}`: {v}')
for title,vals in [('Сводка monitoring handoff (`handoff_monitoring_summary`)',handoff_monitoring_summary),('Правила continuity сигналов (`handoff_signal_continuity_rules`)',handoff_signal_continuity_rules),('Правила traceability handoff (`handoff_traceability_rules`)',handoff_traceability_rules),('Инварианты handoff (`handoff_invariants`)',handoff_invariants),('Правила валидации (`validation_rules`)',validation_rules),('Правила отклонения (`rejection_rules`)',rejection_rules)]:
    md.extend(['',f'## {title}'])
...
outm.write_text('\n'.join(md)+'\n',encoding='utf-8')
print('Готово: сформирован governance monitoring handoff packet в режиме read-only/design-only.')
