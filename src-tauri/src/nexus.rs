...
async fn search_isapi_with_ports(
    ...
    let all_endpoints = vec![
        ...
        (80, format!("http://{}:80/ISAPI/ContentMgmt/search", host)),
        (8080, format!("http://{}:8080/ISAPI/ContentMgmt/search", host)),
    ];
    // Фильтруем по открытым портам чтобы не тратить время на таймауты
    let endpoints: Vec<String> = match open_ports {
        Some(op) => all_endpoints.into_iter().filter(|(p, _)| op.contains(p)).map(|(_, e)| e).collect(),
        None => all_endpoints.into_iter().map(|(_, e)| e).collect(),
    };
    if endpoints.is_empty() {
        let _ = tx.send(HyperionEvent::NexusLog { message: "⚠️ [SEARCH] Нет открытых портов для ContentMgmt/search".into() }).await;
        return vec![];
    }
    let _ = tx.send(HyperionEvent::NexusLog { message: format!("🥷 [SEARCH] Endpoints: {:?}", endpoints) }).await;
    ...
    for endpoint in &endpoints {
        ...
        // === PROBE: Определяем правильный XML формат на первом TID ===
        ...
        for (gen_name, gen_fn) in &xml_generators {
            sand_sleep(&mut rng, sand.api_delay_min_ms, sand.api_delay_max_ms, tx, &format!("probe XML:{}", gen_name)).await;
            ...
            let client = match client { Ok(c) => c, Err(_) => continue };
            let resp = digest_request(&client, reqwest::Method::POST, endpoint, &path, login, pass, Some(xml), tx).await;
            ...
            match resp {
                Ok(r) => {
                    ...
                    if status == 200 {
                        let _ = tx.send(HyperionEvent::NexusLog {
                            message: format!("✅ [PROBE] XML формат '{}' работает!", gen_name),
                        }).await;
                    ...
                    let body = r.text().await.unwrap_or_default();
                    let _ = tx.send(HyperionEvent::NexusLog {
                        message: format!("🧪 [PROBE] {} → HTTP {} | {}", gen_name, status, body.chars().take(200).collect::<String>()),
                    }).await;
                }
                Err(e) => {
                    let _ = tx.send(HyperionEvent::NexusLog {
                        message: format!("🧪 [PROBE] {} → ERR: {}", gen_name, e),
                    }).await;
                }
            }
        }
        ...
        // === ОСНОВНОЙ ПОИСК: используем найденный формат для всех TID ===
        for tid in &track_ids {
            ...
            if let Some(resp) = resp_result {
                ...
                let _ = tx.send(HyperionEvent::NexusLog {
                    message: format!("📡 [TID:{}] HTTP {} | {} chars | body: {}", tid, status, text.len(),
