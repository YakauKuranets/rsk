...
fn get_runtime_logs(
    ...
    // Сортируем по timestamp (формат [HH:MM:SS])
    all_logs.sort();
...
async fn probe_nvr_protocols(
    ...
    if clean_host.is_empty() { return Err("Пустой host для проверки протоколов".into()); }

    push_runtime_log(&log_state, format!("Protocol probe started for {}", clean_host));
    ...
    for endpoint in onvif_endpoints {
        ...
        out.push(ProtocolProbeResult { protocol: "ONVIF".into(), endpoint, status: status.into() });
    }
    ...
    for endpoint in isapi_endpoints {
        let status = match client.get(&endpoint).basic_auth(login.clone(), Some(pass.clone())).send().await {
            Ok(resp) if resp.status().is_success() || resp.status().as_u16() == 401 => "detected",
            Ok(_) => "not_detected",
            Err(_) => "unreachable",
        };
        out.push(ProtocolProbeResult { protocol: "ISAPI".into(), endpoint, status: status.into() });
    }

    let detected = out.iter().filter(|x| x.status == "detected").count();
    push_runtime_log(&log_state, format!("Protocol probe finished for {} (detected: {})", clean_host, detected));
    Ok(out)
}
...
async fn fetch_nvr_device_info(
    ...
    let clean_host = normalize_host_for_scan(&host);
    if clean_host.is_empty() { return Err("Пустой host для ISAPI deviceInfo".into()); }
    ...
    push_runtime_log(&log_state, format!("ISAPI deviceInfo fetch started for {}", clean_host));

    for endpoint in candidates {
        let resp = client.get(&endpoint).basic_auth(login.clone(), Some(pass.clone())).send().await;
        match resp {
            Ok(r) => {
                ...
                if status_code == 200 || status_code == 401 {
                    ...
                    push_runtime_log(&log_state, format!("ISAPI deviceInfo response {} from {}", status_code, endpoint));
                    return Ok(NvrDeviceInfoResult { endpoint, status: status_code.to_string(), body_preview: preview });
                }
            }
            Err(_) => {}
        }
    }
    push_runtime_log(&log_state, format!("ISAPI deviceInfo unavailable for {}", clean_host));
...
async fn fetch_onvif_device_info(
    ...
    let clean_host = normalize_host_for_scan(&host);
    if clean_host.is_empty() { return Err("Пустой host для ONVIF device info".into()); }
    ...
    let soap = r#"<?xml version="1.0" encoding="UTF-8"?><s:Envelope xmlns:s="http://www.w3.org/2003/05/soap-envelope" xmlns:tds="http://www.onvif.org/ver10/device/wsdl"><s:Body><tds:GetDeviceInformation/></s:Body></s:Envelope>"#;

    push_runtime_log(&log_state, format!("ONVIF device info fetch started for {}", clean_host));

    for endpoint in candidates {
        let resp = client.post(&endpoint).header("Content-Type", "application/soap+xml; charset=utf-8").basic_auth(login.clone(), Some(pass.clone())).body(soap.to_string()).send().await;
        match resp {
            Ok(r) => {
                let status_code = r.status().as_u16();
                let text = r.text().await.unwrap_or_default();
                if status_code == 200 || status_code == 401 {
                    let preview = text.chars().take(600).collect::<String>();
                    push_runtime_log(&log_state, format!("ONVIF device info response {} from {}", status_code, endpoint));
                    return Ok(NvrDeviceInfoResult { endpoint, status: status_code.to_string(), body_preview: preview });
                }
            }
            Err(_) => {}
    ...
    push_runtime_log(&log_state, format!("ONVIF device info unavailable for {}", clean_host));
...
async fn probe_archive_export_endpoints(
    ...
    let candidates: Vec<(String, String, String)> = vec![
        ("ISAPI".into(), "GET".into(), format!("http://{}:80/ISAPI/ContentMgmt/search", clean_host)),
        ("ISAPI".into(), "GET".into(), format!("http://{}:80/ISAPI/ContentMgmt/record/tracks", clean_host)),
        ("ISAPI".into(), "GET".into(), format!("https://{}:443/ISAPI/ContentMgmt/search", clean_host)),
        ("ISAPI".into(), "GET".into(), format!("https://{}:443/ISAPI/ContentMgmt/record/tracks", clean_host)),
        ("ONVIF".into(), "POST".into(), format!("http://{}:80/onvif/recording_service", clean_host)),
        ("ONVIF".into(), "POST".into(), format!("http://{}:8080/onvif/recording_service", clean_host)),
        ("ONVIF".into(), "POST".into(), format!("https://{}:443/onvif/recording_service", clean_host)),
        ("ONVIF".into(), "POST".into(), format!("https://{}:8443/onvif/recording_service", clean_host)),
        ("ISAPI".into(), "GET".into(), format!("http://{}:2019/ISAPI/ContentMgmt/search", clean_host)),
        ("ISAPI".into(), "GET".into(), format!("http://{}:2019/ISAPI/ContentMgmt/record/tracks", clean_host)),
        ("ONVIF".into(), "POST".into(), format!("http://{}:2019/onvif/recording_service", clean_host)),
    ];
  ...
  <s:Body>
    ...
    for (protocol, method, endpoint) in candidates {
        let resp = if method == "GET" {
            client.get(&endpoint).basic_auth(login.clone(), Some(pass.clone())).send().await
        } else {
    ...
    push_runtime_log(
        &log_state,
        format!("Archive endpoint probe finished for {} (detected: {})", clean_host, detected),
    );
...
async fn spider_full_scan(
    ...
    let mut crypto_formulas: Vec<String> = Vec::new(); // <--- Наша новая копилка алгоритмов

    // Регулярки для поиска формулы хэширования
    let crypto_patterns = vec![
        Regex::new(r#"(?i)([a-z0-9_.]*sha256\s*\([^;]+;)"#).unwrap(),
        Regex::new(r#"(?i)(password\s*=\s*[^;]+(salt|challenge)[^;]+;)"#).unwrap(),
        Regex::new(r#"(?i)(CryptoJS\.[a-zA-Z0-9_]+\([^;]+;)"#).unwrap(),
        Regex::new(r#"(?i)(encrypt\s*\([^;]+;)"#).unwrap(),
    ];
    ...
    for script_url in &all_scripts {
        ...
        if let Ok(resp) = req.send().await {
            if let Ok(js_body) = resp.text().await {
                ...
                // 2. Ищем криптографию (наша новая фича!)
                for pattern in &crypto_patterns {
                    for cap in pattern.captures_iter(&js_body) {
                        if let Some(m) = cap.get(1) {
                            let match_str = m.as_str().replace('\n', " ").trim().to_string();
                            let result = format!("📍 [{}]: {}", js_name, match_str);
                            if !crypto_formulas.contains(&result) {
                                crypto_formulas.push(result);
                            }
                        }
                    }
                }
    ...
    if !crypto_formulas.is_empty() {
        push_runtime_log(&log_state, format!("🎯 Найдено {} фрагментов криптографии!", crypto_formulas.len()));
    }
...
#[tauri::command]
async fn nemesis_extract_crypto_logic(
    host: String,
    log_state: tauri::State<'_, LogState>
) -> Result<Vec<String>, String> {
    let clean_host = normalize_host_for_scan(&host);
    if clean_host.is_empty() { return Err("Пустой хост".into()); }

    push_runtime_log(&log_state, format!("🧠 Запуск авто-анализатора JS криптографии для: {}", clean_host));
    ...
    // 1. Пробуем разные точки входа для логина
    let login_urls = vec![
        format!("http://{}:2019/doc/page/login.asp", clean_host),
        format!("http://{}:80/doc/page/login.asp", clean_host),
    ...
    if html_body.is_empty() {
        return Err("Не удалось загрузить страницу авторизации камеры".into());
    }
    ...
    for cap in script_re.captures_iter(&html_body) {
        if let Some(m) = cap.get(1) {
            ...
            if link.starts_with("../") {
                link = format!("http://{}:2019/doc/{}", clean_host, link.trim_start_matches("../"));
            } else if link.starts_with('/') {
                link = format!("http://{}:2019{}", clean_host, link);
            } else if !link.starts_with("http") {
                // Если относительная в текущей папке
                let base_dir = base_url.rsplit_once('/').map(|(path, _)| path).unwrap_or(&base_url);
                link = format!("{}/{}", base_dir, link);
            }
            js_links.push(link);
        }
    }

    js_links.sort(); js_links.dedup();
    push_runtime_log(&log_state, format!("  📥 Найдено {} JS файлов. Выкачиваем...", js_links.len()));

    // 3. Выкачиваем JS и ищем алгоритмы
    // Регулярки для поиска формулы хэширования
    ...
    let mut found_formulas = Vec::new();

    for js_url in js_links {
        if let Ok(resp) = client.get(&js_url).send().await {
            if let Ok(js_code) = resp.text().await {
                // Ищем совпадения по регуляркам
                for pattern in &target_patterns {
                    for cap in pattern.captures_iter(&js_code) {
                        if let Some(m) = cap.get(1) {
                            let match_str = m.as_str().replace('\n', " ").trim().to_string();
                            let result = format!("📍 [{}]: {}", js_url.split('/').last().unwrap_or("?"), match_str);
                            if !found_formulas.contains(&result) {
                                found_formulas.push(result);
                            }
                        }
                    }
                }
    ...
    if found_formulas.is_empty() {
        push_runtime_log(&log_state, "❌ Специфичных крипто-формул не найдено.".to_string());
        Ok(vec!["Алгоритм не найден в открытом виде (возможно обфусцирован)".to_string()])
    } else {
        push_runtime_log(&log_state, format!("🎯 Найдено {} потенциальных алгоритмов шифрования!", found_formulas.len()));
        Ok(found_formulas)
    }
}
...
async fn run_nexus_protocol(
    ...
    // Кидаем событие в Шину (Мастеру) с полными данными
    ...
    // Мгновенно возвращаем ответ интерфейсу
    Ok(serde_json::json!({
...
fn main() {
    ...
    // 🔥 2.5. МОСТ ЛОГОВ: nexus -> LogState
    // Логи из nexus приходят через mpsc канал, мы пишем их в LogState
    ...
    std::thread::spawn(move || {
        ...
        rt.block_on(async {
            ...
            while let Some(msg) = rx.recv().await {
                ...
                println!("{}", line); // дублируем в stdout
    ...
    tauri::Builder::default()
        ...
        .manage(nexus_log_shared) // <--- МОСТ ЛОГОВ NEXUS
        .manage(StreamState {
