...
fn start_stream(
    ...
    let child = Command::new(get_vault_path().join("ffmpeg.exe"))
        .args([
            // --- ТРАНСПОРТ ---
            "-rtsp_transport", "tcp",
            "-timeout", "10000000",        // 10 сек таймаут на подключение (в микросекундах)
            "-stimeout", "10000000",       // 10 сек таймаут на чтение сокета
            // --- РЕКОННЕКТ (для HTTP/MJPEG источников) ---
            "-reconnect", "1",
            "-reconnect_at_eof", "1",
            "-reconnect_streamed", "1",
            "-reconnect_delay_max", "5",
            ...
            "-tune", "zerolatency",
            "-crf", "28",
            "-g", "30",                     // Keyframe каждые 30 кадров — быстрый seek
            "-sc_threshold", "0",           // Отключаем сцен-детект для стабильных GOP
            "-an",                          // Без аудио
            // --- HLS ---
            "-f", "hls",
            "-hls_time", "2",              // 2 сек сегменты (стабильнее чем 1 сек)
            "-hls_list_size", "5",         // 5 сегментов в плейлисте
            "-hls_flags", "delete_segments+append_list+omit_endlist",
            "-hls_segment_type", "mpegts",
            playlist.to_str().unwrap(),
        ])
        .stdout(Stdio::null())
        .stderr(Stdio::piped())            // Перехватываем stderr для диагностики
        .spawn()
        .map_err(|e| e.to_string())?;
    state
        .active_streams
...
<SearchDescription>
  ...
  <metadataList><metadataDescriptor>//recordType.meta.std-cgi.com</metadataDescriptor></metadataList>
    ...
    for endpoint in candidates {
        ...
        let text = if is_2019 {
            // Порт 2019: Digest auth — bootstrap POST без auth → 401 → Digest → POST с auth
            let boot = client.post(&endpoint)
            ...
            match boot {
                Ok(r) if r.status().as_u16() == 401 => {
                    let www_auth = r.headers().get(reqwest::header::WWW_AUTHENTICATE)
                    ...
                    if let Some(ah) = www_auth {
                        ...
                        if let Ok(mut prompt) = digest_auth::parse(&ah) {
                            let mut ctx = digest_auth::AuthContext::new(login.clone(), pass.clone(), path);
                            ctx.method = digest_auth::HttpMethod::POST;
                            if let Ok(answer) = prompt.respond(&ctx) {
                                let resp2 = client.post(&endpoint)
                                    .header("Authorization", answer.to_string())
                                    .header("Content-Type", "application/xml; charset=UTF-8")
                                    .header("X-Requested-With", "XMLHttpRequest")
                                    .header("User-Agent", "Mozilla/4.0 (compatible; MSIE 8.0; Windows NT 6.1; WOW64; Trident/4.0)")
                                ...
                                match resp2 {
                                    Ok(r2) => {
                                        ...
                                        push_runtime_log(&log_state, format!("ISAPI search Digest :2019 → HTTP {} ({} chars)", code, t.len()));
                                        if code == 200 { Some(t) } else { None }
                                    }
                                    Err(e) => { push_runtime_log(&log_state, format!("ISAPI search Digest error: {}", e)); None }
                                }
                            } else { None }
                        } else { None }
                    } else { None }
                ...
                Ok(r) => { let _ = r.text().await; None }
                Err(e) => { push_runtime_log(&log_state, format!("ISAPI search :2019 error: {}", e)); None }
            }
        } else {
            // Стандартные порты: Basic auth
            let resp = client
                .post(&endpoint)
        ...
        if let Some(text) = text {
                ...
                let uris: Vec<String> = uri_re
                    ...
                    .filter_map(|c| c.get(1).map(|m| {
                        // Нормализация: XML может экранировать & как &amp;
