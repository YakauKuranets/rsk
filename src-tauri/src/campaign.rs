...
pub fn create_campaign(
    ...
    c.timeline.push(TimelineEvent {
        timestamp: now,
        event_type: "campaign_created".into(),
        description: "Campaign created".into(),
        actor: "system".into(),
    });
    save_campaign(&c)?;
    Ok(c)
...
#[tauri::command]
pub fn get_campaign(campaign_id: String) -> Result<CampaignProject, String> {
...
#[tauri::command]
pub fn add_campaign_finding(
    campaign_id: String,
    finding_json: String,
) -> Result<CampaignFinding, String> {
    let mut c = load_campaign(&campaign_id)?;
    ...
    c.timeline.push(TimelineEvent {
        timestamp: c.updated_at.clone(),
        event_type: "finding_added".into(),
        description: format!("{} ({})", finding.title, finding.severity),
        actor: finding.found_by.clone(),
    });
    save_campaign(&c)?;
    Ok(finding)
}
...
pub fn import_scan_results(
    ...
    c.timeline.push(TimelineEvent {
        timestamp: Utc::now().to_rfc3339(),
        event_type: "playbook_run".into(),
        description: format!("Imported {} findings from {}", count, module_name),
        actor: "system".into(),
    });
    c.updated_at = Utc::now().to_rfc3339();
    save_campaign(&c)?;
...
pub async fn export_campaign_report(
    campaign_id: String,
    format: String,
    _log_state: State<'_, crate::LogState>,
