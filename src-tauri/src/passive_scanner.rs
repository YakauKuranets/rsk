...
fn get_oui_database() -> HashMap<&'static str, &'static str> {
    ...
    db.insert("00:09:FB", "Philips Medical");
    db.insert("00:17:C4", "Siemens Medical");
    db.insert("00:1E:8F", "GE Healthcare");
    db.insert("00:50:F2", "Draeger Medical");
    db.insert("00:80:25", "Becton Dickinson");
    db.insert("28:57:BE", "Hangzhou Hikvision");
    db.insert("54:C4:15", "Hangzhou Hikvision");
    db.insert("BC:AD:28", "Hangzhou Hikvision");
    db.insert("3C:EF:8C", "Dahua Technology");
    db.insert("A0:BD:1D", "Dahua Technology");
    db.insert("AC:CC:8E", "Axis Communications");
    db.insert("00:1A:2B", "Cisco");
    db.insert("00:50:56", "VMware");
    db.insert("B8:27:EB", "Raspberry Pi");
    db.insert("DC:A6:32", "Raspberry Pi");
    db
}
...
fn classify_device(
    ...
    if ["hikvision", "dahua", "axis"].iter().any(|x| m.contains(x)) {
        return "camera".into();
    }
    if open_ports.contains(&554) || b.contains("rtsp") {
        return "camera".into();
    }
    ...
    if open_ports.contains(&80)
        && (b.contains("router") || b.contains("mikrotik") || b.contains("routeros"))
    {
        return "router".into();
    }
    "unknown".into()
...
async fn lookup_cves_for_device(
    manufacturer: &str,
    firmware: Option<&str>,
    _device_type: &str,
) -> Vec<KnownCve> {
    let query = if let Some(fw) = firmware {
        format!("{} {}", manufacturer, fw)
    } else {
        manufacturer.to_string()
    };
    let local =
        crate::vuln_db_updater::query_local_vuln_db(manufacturer.to_string(), query.clone())
            .await
            .unwrap_or_default();
    let mut out: Vec<KnownCve> = local
        .into_iter()
        .take(10)
        .map(|x| {
...
fn expand_subnet(target_subnet: &str) -> Result<Vec<String>, String> {
    let net: IpNetwork = target_subnet
        .parse()
    ...
    if let Ok(out) = output {
        ...
        for line in s.lines() {
            if let (Some(ip), Some(mac)) = (ip_re.find(line), mac_re.find(line)) {
                map.insert(
                    ip.as_str().to_string(),
                    mac.as_str().replace('-', ":").to_uppercase(),
                );
            }
        }
    }
...
async fn banner_grab(ip: &str, port: u16) -> Option<String> {
    ...
    let mut stream = timeout(Duration::from_millis(300), TcpStream::connect(&addr))
        .await
        .ok()?
        .ok()?;
    ...
    if port == 554 {
        let _ = stream
            .write_all(b"OPTIONS rtsp://example.com RTSP/1.0\r\nCSeq: 1\r\n\r\n")
            .await;
    }
...
#[tauri::command]
pub async fn passive_scan_network(
    target_subnet: String,
    _scan_depth: Option<String>,
    log_state: State<'_, crate::LogState>,
) -> Result<PassiveScanReport, String> {
    crate::push_runtime_log(
        &log_state,
        format!("[PASSIVE] scan subnet {}", target_subnet),
    );
    let started = Instant::now();
    let ips = expand_subnet(&target_subnet)?;
    let arp = parse_arp_table();
    let ports: Vec<u16> = vec![
        80, 443, 554, 8080, 21, 22, 23, 161, 502, 1883, 47808, 9100, 104,
    ];
        ...
        .map(|ip| {
            ...
            async move {
                ...
                for p in ports {
                    ...
                    {
                        open_ports.push(p);
                        if let Some(b) = banner_grab(&ip, p).await {
                            banners.insert(p, b);
                        }
                    }
                }
                ...
                let device_type =
                    classify_device(manufacturer.as_deref().unwrap_or(""), &banners, &open_ports);
                let known_cves = lookup_cves_for_device(
                    manufacturer.as_deref().unwrap_or("Unknown"),
                    None,
                    &device_type,
                )
                .await;
                let max_cvss = known_cves
                    .iter()
                    .filter_map(|c| c.cvss_score)
                    .fold(0.0f32, f32::max);
                let risk_level = if max_cvss >= 9.0 {
                    "critical"
    ...
    let medical_devices: Vec<IoTDevice> = devices
        .iter()
        .filter(|d| d.device_type == "medical")
        .cloned()
        .collect();
...
#[tauri::command]
pub async fn analyze_pcap_file(
    pcap_path: String,
    log_state: State<'_, crate::LogState>,
) -> Result<PassiveScanReport, String> {
    ...
    for line in text.lines() {
        ...
        let d = by_ip.entry(src.to_string()).or_insert(IoTDevice {
            ...
            detection_method: "pcap".into(),
        });
        if port > 0 && !d.open_ports.contains(&port) {
            d.open_ports.push(port);
        }
        if !http_server.is_empty() {
            d.banners.insert(80, http_server.to_string());
        }
        if !rtsp_server.is_empty() {
            d.banners.insert(554, rtsp_server.to_string());
        }

        if port == 21 {
            warnings.push(format!(
                "Обнаружена передача учётных данных в открытом виде (FTP) между {} и {}",
                src, dst
            ));
        }
        if port == 23 {
            warnings.push(format!(
                "Обнаружена передача учётных данных в открытом виде (Telnet) между {} и {}",
                src, dst
            ));
        }
    }
    ...
    for d in &mut devices {
        ...
        let max_cvss = d
            ...
            .filter_map(|c| c.cvss_score)
            .fold(0.0f32, f32::max);
        d.risk_level = if max_cvss >= 9.0 {
            "critical".into()
        } else if max_cvss >= 7.0 {
            "high".into()
        } else if max_cvss >= 4.0 {
            "medium".into()
        } else if !d.open_ports.is_empty() {
            "low".into()
        } else {
            "info".into()
        };
